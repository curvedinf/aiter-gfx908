...
def fused_moe_2stages(
    ...
    if (
        ...
        if get_gfx() == "gfx1250":
            # Unreachable now (guarded above); kept for grep/back-compat.
        ...
        if hidden_states.dtype == q_dtype_a and a1_scale is not None:
            ...
            # NOTE: prefer the HIP kernel (per_1x32_f4_quant_hip) over the
            # pure torch reference (per_1x32_f4_quant): on warmup with M=16384
            # the torch path takes 10+ minutes (it builds a `.float()`
            # reduction tree over hundreds of millions of elements) and
            # the user perceives it as a hang.  The HIP kernel produces the
            # same source-token layout the FlyDSL stage1 wrapper expects.
            from aiter.ops.quant import per_1x32_f4_quant_hip
            a1_flat = hidden_states.view(-1, hidden_states.shape[-1])
            a1, a1_scale = per_1x32_f4_quant_hip(
                a1_flat, quant_dtype=dtypes.fp4x2, shuffle=False
            )
