...
def fused_moe(
    ...
    # fast path for small batches (PR-1747: torch ops + prebuilt .co, no pyhip)
    if os.environ.get("AITER_MOE_SMALL_BATCH", "0") == "1" and hidden_states.shape[0] <= 16 and hidden_states.dtype == torch.bfloat16 and expert_mask is None and activation == ActivationType.Silu and (
        ...
        gemm1_out = torch.empty(
            [B, TOPK, N1 // 2], dtype=hidden_states.dtype, device=hidden_states.device
        )
        empty_scale = torch.empty((0, 1), dtype=torch.bfloat16, device=hidden_states.device)
        if B == 1:
            assert N1 == 2 * K2
            ...
            moe_stage1_g1u1_small_batch1(
                ...
                topk_ids,
                topk_weight,
                w1_scale if w1_scale is not None else empty_scale,
            )
            moe_stage2_g1u1_small_batch1(
                gemm1_out,
                w2,
                ...
                topk_ids,
                topk_weight,
                w2_scale if w2_scale is not None else empty_scale,
            )
            return gemm2_out
        BLOCK_M = 16
