...
def fused_moe_2stages(
    ...
    if metadata.fuse_quant == "fp4" and isinstance(a2, tuple):
        ...
        # See stage1 comment: triton kernel kept for cuda-graph compat,
        # supersedes our earlier hip migration (which itself replaced the
        # 10-minute pure-torch fallback at warmup-sized M).
        from aiter.ops.quant import per_1x32_f4_quant_triton
        a2_flat = a2.view(-1, inter_dim)
