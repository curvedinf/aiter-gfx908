...
def _get_shared_shuffle_state(w1: torch.Tensor, w2: torch.Tensor) -> bool:
    """MoE gate/up weights must agree on whether they are preshuffled."""
    w1_is_shuffled = getattr(w1, "is_shuffled", False)
    w2_is_shuffled = getattr(w2, "is_shuffled", False)
    if w1_is_shuffled != w2_is_shuffled:
        raise ValueError(
            "MoE gate/up weights must share the same preshuffled state: "
            f"w1.is_shuffled={w1_is_shuffled}, w2.is_shuffled={w2_is_shuffled}"
        )
    return w1_is_shuffled
...
def fused_moe_(
    ...
    isShuffled = _get_shared_shuffle_state(w1, w2)
...
def get_2stage_cfgs(
    ...
    if (
        ...
        raise RuntimeError(
            "Generic GPT-OSS SwiGLU MXFP4 requires the FlyDSL fp4 path. "
            "Refusing unsupported fallback to CK2stages; check FlyDSL "
            "availability and stage1 weighting configuration."
        )
    if (
        dtype in [dtypes.bf16, dtypes.fp16]
        and q_type == QuantType.per_1x32
...
def fused_moe_2stages(
    ...
    is_shuffled = _get_shared_shuffle_state(w1, w2)
    metadata = get_2stage_cfgs(
