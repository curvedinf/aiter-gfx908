...
import argparse
import subprocess
from aiter.ops.triton._triton_kernels.attention.fav3_sage_attention import sage_quant
import pytest
import sys
import torch
...
class MetaData():
    ...
    def set_int8_params(self, q_descale, k_descale, v_descale, bias):
        self.int8 = True
...
def _attn_fwd_inner(acc, l_i, m_i, q, k_ptrs, v_ptrs, bias_ptrs, stride_kn, stride_vk, stride_bn, start_m,
                    ...
                    k_descale_base_ptrs, stride_ksn, IS_CAUSAL: tl.constexpr, BLOCK_M: tl.constexpr,
...
def attn_fwd(Q, K, V, bias, SM_SCALE: tl.constexpr, L, Out, stride_qz, stride_qh, stride_qm, stride_qk, stride_kz,
             ...
             stride_om, stride_on, stride_bz, stride_bh, stride_bm, stride_bn, stride_az, stride_ah, 
             stride_qsz, stride_qsh, stride_qsm, stride_qsk, stride_ksz, stride_ksh, stride_ksn, stride_ksk,
        ...
        if continue_condition:
            ...
            if (IS_CAUSAL):
                ...
                if n_blocks <= 0:
                    ...
                    o_ptrs = o_offset + offs_m[:, None] * stride_om + offs_dv[None, :] * stride_on
            ...
            if continue_condition:
                ...
                q_descale_offset = Q_descale + off_z * stride_qsz + off_h_q * stride_qsh
                q_descale_ptrs = q_descale_offset + start_m * stride_qsm
                
                k_descale_offset = K_descale + off_z * stride_ksz + off_h_k * stride_ksh
                k_descale_ptrs = k_descale_offset 
                ...
                if n_full_blocks > 0:
                    ...
                    acc, l_i, m_i = _attn_fwd_inner(acc, l_i, m_i, q, k_ptrs, v_ptrs, bias_ptrs, stride_kn, stride_vk,
                                                    ...
                                                    block_min, block_max, 0, 0, 0, alibi_slope, q_descale, k_descale_ptrs, stride_ksn,
                                                    # IS_CAUSAL, ....
                                                    False, BLOCK_M, BLOCK_DMODEL, BLOCK_N, offs_m, offs_n,
                                                    # _, MASK_STEPS, ...
                                                    PRE_LOAD_V, False, ENABLE_DROPOUT, RETURN_ENCODED_SOFTMAX,
                                                    PADDED_HEAD, ACTUAL_BLOCK_DMODEL, QK_SCALE, True)
                    block_min = block_max
                    block_max = n_blocks * BLOCK_N
                ...
                if (masked_blocks > 0):
                    ...
                    k_descale_ptrs += n_full_blocks * stride_ksn
                    ...
                    acc, l_i, m_i = _attn_fwd_inner(
                        ...
                        offs_n_causal, masked_blocks, n_extra_tokens, alibi_slope, q_descale, k_descale_ptrs, stride_ksn, IS_CAUSAL, BLOCK_M, BLOCK_DMODEL, BLOCK_N, offs_m, offs_n,
                        # _, MASK_STEPS, ...
...
def quantize_input(
    ...
    input_metadata.set_int8_params(q_descale=q_descale, k_descale=k_descale, v_descale=v_descale, bias=None)

    return q_int8, k_int8, v_fp8
