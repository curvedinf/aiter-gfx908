...
from flydsl.runtime.device import (
    get_rocm_arch as _get_rocm_arch,
    is_rdna_arch as _is_rdna_arch,
)
...
WARP_SIZE = 32 if _is_rdna_arch(_get_rocm_arch()) else 64
...
def build_fused_rope_cache_module(
    ...
    if head_dim % VEC_WIDTH != 0:
        raise ValueError(
            f"head_dim must be a multiple of VEC_WIDTH ({VEC_WIDTH}), got {head_dim}"
        )
    if rotary_dim % 2 != 0:
        raise ValueError(f"rotary_dim must be even, got {rotary_dim}")
    if half_dim % VEC_WIDTH != 0:
        raise ValueError(
            f"half_dim must be a multiple of VEC_WIDTH ({VEC_WIDTH}), got {half_dim}"
        )
    if not flash_layout and head_dim % x_size != 0:
        raise ValueError(
            f"head_dim must be a multiple of x_size ({x_size}), got {head_dim}"
        )
    ...
    def fused_qk_rope_reshape_and_cache(
        ...
        def ds_bpermute_pair(vec_val, pair_byte_addr):
            ...
            if VEC_WIDTH == 1:
                # vector<1xf16/bf16> (16-bit) → extract scalar → bitcast to i16 → zero-extend i32
                elem_val = vector.extract(
                    vec_val, static_position=[0], dynamic_position=[]
                )
                ...
                for ci in range_constexpr(n_i32):
                    chunk = vector.extract(
                        v_i32, static_position=[ci], dynamic_position=[]
                    )
                    peer_chunks.append(
                        fx.rocdl.ds_bpermute(T.i32, pair_byte_addr, chunk)
                    )
        ...
        if tid < fx.Int32(vecs_per_head):
            ...
            if pos_dtype == "i64":
                ...
                pos_elem_off = pid_t
            pos_val = buffer_ops.buffer_load(
                pos_rsrc, pos_elem_off, vec_width=1, dtype=T.i32
            )
            ...
            if head_idx < fx.Int32(num_kv_heads_val):
                ...
                # --- KV Cache write ---
                slot_rsrc = buffer_ops.create_buffer_resource(
                    SlotMapping, max_size=True
                )
                if pos_dtype == "i64":
                    slot_elem_off = ArithValue(pid_t) * 2
                else:
                    slot_elem_off = pid_t
                slot_val = buffer_ops.buffer_load(
                    slot_rsrc, slot_elem_off, vec_width=1, dtype=T.i32
                )
                ...
                if slot_val >= fx.Int32(0):
                    ...
                    if apply_scale:
                        ...
                        ks_div = fx.logical_divide(
                            fx.slice(ks_buf, (None,)), f32_reg_lay
                        )
                        vs_div = fx.logical_divide(
                            fx.slice(vs_buf, (None,)), f32_reg_lay
                        )
                        r_ks = fx.memref_alloca(f32_reg_ty, f32_reg_lay)
                        r_vs = fx.memref_alloca(f32_reg_ty, f32_reg_lay)
                        fx.copy_atom_call(
                            f32_copy_atom, fx.slice(ks_div, (None, fx.Int32(0))), r_ks
                        )
                        fx.copy_atom_call(
                            f32_copy_atom, fx.slice(vs_div, (None, fx.Int32(0))), r_vs
                        )
                        k_scale_val = vector.extract(
                            fx.memref_load_vec(r_ks),
                            static_position=[0],
                            dynamic_position=[],
                        )
                        v_scale_val = vector.extract(
                            fx.memref_load_vec(r_vs),
                            static_position=[0],
                            dynamic_position=[],
                        )
                        ...
                        kc_fp8_rsrc = buffer_ops.create_buffer_resource(
                            KeyCache, max_size=True
                        )
                        vc_fp8_rsrc = buffer_ops.create_buffer_resource(
                            ValueCache, max_size=True
                        )
                        ...
                        if VEC_WIDTH >= 4:
                            ...
                            if flash_layout:
                                ...
                                for wi in range_constexpr(VEC_WIDTH // 4):
                                    buffer_ops.buffer_store(
                                        k_fp8[wi], kc_fp8_rsrc, kc_dw + fx.Int32(wi)
                                    )
                                    buffer_ops.buffer_store(
                                        v_fp8[wi], vc_fp8_rsrc, kc_dw + fx.Int32(wi)
                                    )
                            else:
                                ...
                                for vi in range_constexpr(VEC_WIDTH):
                                    ...
                                    i32_idx = vi // 4
                                    byte_in_i32 = vi % 4
                                    shifted = ArithValue(v_fp8[i32_idx]) >> (
                                        byte_in_i32 * 8
                                    )
                                    fp8_byte = arith.trunci(T.i8, shifted)
                                    buffer_ops.buffer_store(
                                        fp8_byte, vc_fp8_rsrc, vc_byte_off
                                    )
                        else:
                            ...
                            for vi in range_constexpr(VEC_WIDTH):
                                ...
                                if flash_layout:
                                    ...
                                    buffer_ops.buffer_store(
                                        k_byte, kc_fp8_rsrc, byte_off
                                    )
                                    buffer_ops.buffer_store(
                                        v_byte, vc_fp8_rsrc, byte_off
                                    )
                                else:
                                    ...
                                    buffer_ops.buffer_store(
                                        k_byte, kc_fp8_rsrc, kc_byte_off
                                    )
                                    ...
                                    buffer_ops.buffer_store(
                                        v_byte, vc_fp8_rsrc, vc_byte_off
                                    )
                    else:
                        ...
                        if flash_layout:
                            ...
                            kc_row = fx.slice(
                                KC_buf, (pid_t_slot, pid_b, head_idx, None)
                            )
                            vc_row = fx.slice(
                                VC_buf, (pid_t_slot, pid_b, head_idx, None)
                            )
                            ...
                            kc_rsrc = buffer_ops.create_buffer_resource(
                                KeyCache, max_size=True
                            )
                            vc_rsrc = buffer_ops.create_buffer_resource(
                                ValueCache, max_size=True
                            )
                            for vi in range_constexpr(VEC_WIDTH):
                                d_idx = ArithValue(tid) * VEC_WIDTH + vi
                            ...
                            for vi in range_constexpr(VEC_WIDTH):
                                ...
                                v_elem = vector.extract(
                                    v_e, static_position=[vi], dynamic_position=[]
                                )
