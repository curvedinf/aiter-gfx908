...
def compile_mxscale_gemm(
    *,
    ...
    if stage1_act_mode is not None:
        ...
        if use_tdm_store:
            raise ValueError("stage1_act GEMM epilogue fuse requires use_tdm_store=False")
        if wave_specialized_tdm:
            raise ValueError("stage1_act GEMM epilogue fuse does not support wave_specialized_tdm")
    ...
    if grouped_persistent_m:
        ...
        if _persistent_workers < 1:
            raise ValueError(f"persistent_workers must be >= 1, got {_persistent_workers}")
    else:
    ...
    stage1_act_interleave = stage1_act_mode is not None and stage1_weight_layout_mode == "gugu"
    ...
    TDM_LOADS_PER_STEP = 1 if wave_specialized_tdm else (
        6 if stage1_dual_b else 4
    )
    tail_plan = [
    ...
    def kernel_mxscale_gemm(
        ...
        def _emit_tile(batch_idx, bx_local, by_local, bz, tile_valid_override=None):
            ...
            def make_desc_as(memref, k_base):
                ...
                return tdm_ops.make_tensor_descriptor_2d(
                    ...
                    tensor_shape=(batch_count * (M // wmma_m_rep), K_scale * wmma_m_rep),
            ...
            def make_desc_bs(memref, k_base, n_offset=0):
                k_scale_off = k_base / arith.index(SCALE_BLOCK)
                outer_off = (blk_n + arith.index(n_offset)) / arith.index(b_scale_load_rep)
                ...
                return tdm_ops.make_tensor_descriptor_2d(
                    ...
                    tensor_shape=(batch_count * (B_TOTAL_N // b_scale_load_rep),
            ...
            def _add_bias_vec8(acc_vec8, wn, n_offset=0):
                ...
                if const_expr(split_k > 1):
                    ...
                    for vi in range_constexpr(8):
                        ...
                        elems.append(arith.select(is_first_split, biased_elem, acc_elem))
                    return vector.from_elements(T.vec(8, T.f32), elems)
                return biased
            ...
            def _stage1_act_mul_vec8(gate_v8, up_v8):
                ...
                for vi in range_constexpr(8):
                    ...
                    u = vector.extract(
                        up_v8, static_position=[vi], dynamic_position=[])
                ...
                for acc_idx, vec_base, m_off, wn in _sub_tiles:
                    ...
                    if const_expr(needs_grouped_row_masked_store):
                        ...
                        with ir.InsertionPoint(store_if.then_block):
                            if const_expr(_bf16_out):
                                store_acc_vec8_to_buffer(
                                    out_sub8, c_rsrc, addrs[addr_idx],
                                    out_elem=_out_elem_local, offset_is_bytes=True)
                            else:
                                store_acc_vec8_to_buffer(
            ...
            def epilogue_stage1_act_interleaved_stores(final_accs):
                for acc_idx, vec_base, m_off, wn in _sub_tiles:
                    ...
                    raw_col_base = (blk_n + warp_n_base + arith.index(wn * WMMA_N)
                                    + lane_kgrp * arith.index(8))
                    ...
                    for pair in range_constexpr(4):
                        ...
                        u = vector.extract(
                            raw_sub8, static_position=[pair * 2 + 1], dynamic_position=[])
                        out_vals.append(_stage1_act_mul_scalar(g, u))
                    store_if = scf.IfOp(store_valid, results_=[], has_else=False)
                    with ir.InsertionPoint(store_if.then_block):
                        elem_off = row * arith.index(C_N) + out_col_base
                        if const_expr(_bf16_out):
                            h_vals = [arith.trunc_f(_out_elem_local, v) for v in out_vals]
                            h_vec = vector.from_elements(T.vec(4, _out_elem_local), h_vals)
                            i32_vec = vector.bitcast(T.vec(2, T.i32), h_vec)
                            byte_off = elem_off * arith.index(elem_bytes_d)
                            buffer_ops.buffer_store(
                                i32_vec, c_rsrc, arith.index_cast(T.i32, byte_off),
            ...
            stages_b_up = [
                SmemPtr(arena_base_ptr, stage_b_up_data_off[i], elem_ty_lds,
                        shape=(lds_b_data_f16,))
                for i in range_constexpr(num_buffers)
            ]
            stages_as = [
            ...
            stages_bs_up = [
                SmemPtr(arena_base_ptr, stage_b_up_scale_off[i], elem_ty_lds,
                        shape=(lds_b_scale_f16,))
                for i in range_constexpr(num_buffers)
            ]
            ...
            stages_b_idx = [extract_lds_base_idx(stages_b[i])
                            for i in range_constexpr(num_buffers)]
            stages_b_up_idx = [extract_lds_base_idx(stages_b_up[i])
                               for i in range_constexpr(num_buffers)]
            stages_as_idx = [extract_lds_base_idx(stages_as[i])
                             for i in range_constexpr(num_buffers)]
            stages_bs_idx = [extract_lds_base_idx(stages_bs[i])
                             for i in range_constexpr(num_buffers)]
            stages_bs_up_idx = [extract_lds_base_idx(stages_bs_up[i])
                                for i in range_constexpr(num_buffers)]
            ...
            for i in range_constexpr(num_buffers):
                ...
                if const_expr(stage1_dual_b):
                    stages_b_up_lds_addr.append(vector.extract(
                ...
                stages_bs_lds_addr.append(vector.extract(
                    ...
                    static_position=[1], dynamic_position=[]))
                if const_expr(stage1_dual_b):
                    stages_bs_up_lds_addr.append(vector.extract(
                        make_desc_bs(stages_bs_up_mem[i], arith.index(0), N).dgroup0,
                        static_position=[1], dynamic_position=[]))
            ...
            if const_expr(wave_specialized_tdm):
                ...
                if const_expr(stage1_dual_b):
                    addr_lo_bs_up = vector.extract(
                        desc_bs_up_init.dgroup0, static_position=[2], dynamic_position=[])
                    addr_hi_bs_up = vector.extract(
                        desc_bs_up_init.dgroup0, static_position=[3], dynamic_position=[])
                ...
                for i in range_constexpr(pre_loaded):
                    ...
                    if const_expr(stage1_dual_b):
                        dg0_b_up = vector.from_elements(T.vec(4, T.i32), [
                            pred_const, stages_b_up_lds_addr[i],
                            addr_lo_b_up, addr_hi_b_up])
                        dg0_bs_up = vector.from_elements(T.vec(4, T.i32), [
                            pred_const, stages_bs_up_lds_addr[i],
                            addr_lo_bs_up, addr_hi_bs_up])
                ...
                if const_expr(_outstanding == -1):
                    ...
                    if const_expr(_load_stage is not None):
                        ...
                        if const_expr(wave_specialized_tdm):
                            ...
                            if const_expr(stage1_dual_b):
                                _tail_ab = [[addr_lo_a], [addr_lo_b], [addr_lo_b_up],
                                            [addr_lo_as], [addr_lo_bs], [addr_lo_bs_up]]
                            else:
        ...
        if const_expr(grouped_persistent_m):
            ...
            with ir.InsertionPoint(tile_if.then_block):
                ...
                _emit_tile(batch_idx, bx_local, block_n_id, split_k_id,
    ...
    def launch_mxscale_gemm(
        ...
        launcher = kernel_mxscale_gemm(
            ...
            arg_c, arg_c, arg_c, arg_c, i32_m, i32_n)
    ...
    def launch_mxscale_gemm_masked(
        ...
        launcher = kernel_mxscale_gemm(
            arg_c, arg_a, arg_b, arg_a_scale, arg_b_scale,
            arg_c, arg_masked_m, arg_masked_m, arg_masked_m, i32_m, i32_n)
        for op in ctx.gpu_module_body.operations:
            if const_expr(hasattr(op, 'attributes') and op.OPERATION_NAME == "gpu.func"):
                if const_expr(effective_waves_per_eu is not None):
                    _wpe = int(effective_waves_per_eu)
    ...
    def launch_mxscale_gemm_masked_persistent(
        ...
        launcher = kernel_mxscale_gemm(
            arg_c, arg_a, arg_b, arg_a_scale, arg_b_scale,
            arg_c, arg_masked_m, arg_m_tile_prefix, arg_m_tile_map, i32_m, i32_n)
        for op in ctx.gpu_module_body.operations:
            if const_expr(hasattr(op, 'attributes') and op.OPERATION_NAME == "gpu.func"):
                if const_expr(effective_waves_per_eu is not None):
                    _wpe = int(effective_waves_per_eu)
    ...
    def launch_mxscale_gemm_bias(
        ...
        launcher = kernel_mxscale_gemm(
            arg_c, arg_a, arg_b, arg_a_scale, arg_b_scale,
            arg_bias, arg_c, arg_c, arg_c, i32_m, i32_n)
        for op in ctx.gpu_module_body.operations:
            if const_expr(hasattr(op, 'attributes') and op.OPERATION_NAME == "gpu.func"):
                if const_expr(effective_waves_per_eu is not None):
                    _wpe = int(effective_waves_per_eu)
    ...
    def launch_mxscale_gemm_masked_bias(
        ...
        launcher = kernel_mxscale_gemm(
            arg_c, arg_a, arg_b, arg_a_scale, arg_b_scale,
            arg_bias, arg_masked_m, arg_masked_m, arg_masked_m, i32_m, i32_n)
        for op in ctx.gpu_module_body.operations:
            if const_expr(hasattr(op, 'attributes') and op.OPERATION_NAME == "gpu.func"):
                if const_expr(effective_waves_per_eu is not None):
                    _wpe = int(effective_waves_per_eu)
    ...
    def launch_mxscale_gemm_masked_persistent_bias(
        ...
        launcher = kernel_mxscale_gemm(
            arg_c, arg_a, arg_b, arg_a_scale, arg_b_scale,
            arg_bias, arg_masked_m, arg_m_tile_prefix, arg_m_tile_map, i32_m, i32_n)
        for op in ctx.gpu_module_body.operations:
            if const_expr(hasattr(op, 'attributes') and op.OPERATION_NAME == "gpu.func"):
                if const_expr(effective_waves_per_eu is not None):
                    _wpe = int(effective_waves_per_eu)
    ...
    if epilogue_bias_mode:
        if grouped_masked_m:
            return (
                ...
                if grouped_persistent_m else launch_mxscale_gemm_masked_bias
            )
        return launch_mxscale_gemm_bias
    if grouped_masked_m:
        return launch_mxscale_gemm_masked_persistent if grouped_persistent_m else launch_mxscale_gemm_masked
