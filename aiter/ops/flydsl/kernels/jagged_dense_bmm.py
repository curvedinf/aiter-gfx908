...
@contextmanager
def _if_then(if_op):
    """SCF IfOp then-region (flydsl ``if cond: return`` does not early-exit)."""
...
def compile_jagged_dense_bmm_kernel(
    dtype: str,
    n: int,
    k: int,
    TILE_M: int = 128,
    ...
    """Grouped jagged×dense BMM + optional bias (SPLIT_K=1).

    ``Dense`` must be presented as ``(B, N, K)`` row-major (transpose of genrec
    ``(B, K, N)``). Grid mirrors Triton: ``(N_tiles, M_tiles(max_seq_len), B)``.

    Per-block ``seq_offsets`` row bases are folded into buffer descriptors (i64
    byte bump) so element indices stay within int32 for large ``L``.

    ``XCD_REMAP`` (HipKittens Algorithm 1, chiplet/L2-aware scheduling, gfx942
    has 8 XCDs): when ``>0`` the raw block id is remapped so all tiles of a group
    ``b`` are dispatched to the same XCD (``b % NXCD``), keeping ``Dense[b]``
    resident in that XCD's 4 MB L2 instead of being reloaded into all 8 XCDs.
    The value is the number of consecutive groups assigned to one XCD before
    advancing (group chunk; 1 = pure round-robin). Pure block-index bijection —
    output is identical; requires ``batch_count % NXCD == 0`` else it falls back
    to identity at launch. Most impactful on the HBM-bound, B-heavy N=16384 shapes.
    """
    SPLIT_K = 1
    ...
    # gfx942 MI300X: 8 XCDs (chiplets), each with its own 4 MB L2 slice.
    NXCD = 8
    KERNEL_NAME = (
        f"jdbba_{dtype}_{BLOCK_M}x{BLOCK_N}x{BLOCK_K}x{STAGES}"
        f"_W{BLOCK_M_WARPS}x{BLOCK_N_WARPS}x{BLOCK_K_WARPS}"
        f"_BLDS{int(B_TO_LDS)}_HB{int(HAS_BIAS)}_EW{int(ELEMENTWISE)}"
    )
    KERNEL_NAME += "_AS0" if not ASYNC_COPY else "_AS1"
    if WAVES_PER_EU > 0:
        KERNEL_NAME += f"_wpe{WAVES_PER_EU}"
    # XCD remap only activates with a compile-time M_TILES hint (constexpr
    # divisors → no runtime integer division, which FlyDSL can't lower here).
    XCD_REMAP_ON = (XCD_REMAP > 0) and (M_TILES > 0)
    if XCD_REMAP_ON:
        KERNEL_NAME += f"_xcd{XCD_REMAP}m{M_TILES}"
    ...
    def jdbba_kernel(
        ...
        B: fx.Pointer,
        BIAS: fx.Pointer,
        seq_offsets: fx.Pointer,
        max_seq_len: fx.Int32,
        batch_count: fx.Int32,
        ...
        if const_expr(XCD_REMAP_ON):
            # HipKittens Algorithm 1 (chiplet/L2-aware): remap the raw block id so
            # every tile of a group b lands on one XCD (groups assigned to XCDs in
            # CHUNK-sized runs), keeping Dense[b] resident in that XCD's 4 MB L2
            # instead of being reloaded into all 8 XCDs. HW assigns workgroup
            # flat-id -> XCD round-robin (xcd = flat % NXCD). All divisors are
            # compile-time constants (G, NXCD, CHUNK, N_BLOCKS) — matches the
            # working `tid // WARP_SIZE` pattern; bijection requires
            # batch_count % (NXCD*CHUNK) == 0 (guarded at launch).
            CHUNK = const_expr(int(XCD_REMAP))
            G = const_expr(N_BLOCKS * M_TILES)       # tiles per group (constexpr)
            flat = fx.block_idx.x + N_BLOCKS * (
                fx.block_idx.y + M_TILES * fx.block_idx.z
            )
            xcd = flat % NXCD
            local = flat // NXCD
            gidx = local // G                        # group index within this XCD
            tile_in_group = local % G
            rnd = gidx // CHUNK
            pos = gidx % CHUNK
            b_idx = rnd * (NXCD * CHUNK) + xcd * CHUNK + pos
            block_m_idx = tile_in_group // N_BLOCKS
            block_n_idx = tile_in_group % N_BLOCKS
        else:
            b_idx = fx.block_idx.z
            block_n_idx = fx.block_idx.x
            block_m_idx = fx.block_idx.y
        ks_idx = fx.Index(0)
        ks_begin = arith.constant(0, type=T.i32)

        seq_start = fx.Index(SEQ_[fx.Index(b_idx)])
        seq_end = fx.Index(SEQ_[fx.Index(b_idx) + fx.Index(1)])
        seq_len = seq_end - seq_start
        a_byte_off = seq_start * fx.Index(k * DTYPE_BYTES)
        c_byte_off = seq_start * fx.Index(n * DTYPE_BYTES)
        A_ = GTensor(
            A, dtype=dtype_, shape=(-1, k), static_bytes_offset_i64=a_byte_off
        )
        B_ = GTensor(B, dtype=dtype_, shape=(-1, k))
        C_ = GTensor(
            C, dtype=dtype_, shape=(-1, n), static_bytes_offset_i64=c_byte_off
        )
        if const_expr(HAS_BIAS):
            if const_expr(ELEMENTWISE):
                BIAS_ = GTensor(
                    BIAS,
                    dtype=dtype_,
                    shape=(-1, n),
                    static_bytes_offset_i64=c_byte_off,
                )
            else:
                BIAS_ = GTensor(BIAS, dtype=dtype_, shape=(-1,))

        m_offset = fx.Index(block_m_idx * BLOCK_M)
        n_offset = fx.Index(block_n_idx * BLOCK_N)
        b_row_base = fx.Index(b_idx * n)
        k_blocks16 = fx.Int32(BLOCK_K_BYTES // 16)

        # Triton jagged_dense_bmm_broadcast_add_kernel: return when start_m >= seq_len.
        block_active = arith.cmpi(arith.CmpIPredicate.ult, m_offset, seq_len)
        _if_m_active = scf.IfOp(block_active)
        with _if_then(_if_m_active):
            ...
            def zero_c():
                ...
                with ir.InsertionPoint(cond_ks0_if.then_block):
                    ...
                    for i in range_constexpr(LDG_REG_C_COUNT):
                        ...
                        m_row = m_offset + fx.Index(m_local_idx)
                        init_vec = zero_vec
                        if const_expr(HAS_BIAS):
                            if const_expr(ELEMENTWISE):
                                init_vec = BIAS_.vec_load(
                                    (m_row, n_offset + n_local_idx), LDG_VEC_SIZE
                                )
                            else:
                                init_vec = BIAS_.vec_load(
                                    (b_row_base + n_offset + n_local_idx,), LDG_VEC_SIZE
                                )
                        cond_boundary = arith.cmpi(
                            arith.CmpIPredicate.ult, m_row, seq_len
                        )
                        ...
                        with ir.InsertionPoint(cond_boundary_if.then_block):
                            bytes_offset = C_.linear_offset(
                                (m_row, n_offset + n_local_idx)
                            )
                            c_ptr = get_llvm_ptr(C, bytes_offset, DTYPE_BYTES)
                            llvm.InlineAsmOp(
                                None,
                                [c_ptr, init_vec],
            ...
            def ldg_a(k_offset):
                ...
                for i in range_constexpr(LDG_REG_A_COUNT):
                    ...
                    safe_row_idx = arith.select(
                        arith.cmpi(arith.CmpIPredicate.ult, row_idx, seq_len),
                        row_idx,
                        fx.Index(0),
                    )
            ...
            def ldg_sts_b_async(k_offset, lds_stage):
                for i in range_constexpr(LDG_REG_B_COUNT_AS):
                    ...
                    row_idx = b_row_base + n_offset + fx.Index(n_local_idx)
                    safe_row_idx = arith.select(
                        arith.cmpi(arith.CmpIPredicate.ult, row_idx, b_row_base + fx.Index(n)),
                        row_idx,
                        b_row_base,
                    )
                    col_idx = fx.Index(k_offset + col_in_bytes // DTYPE_BYTES)
            ...
            def ldg_matrix_b(k_offset):
                ...
                for kk in range_constexpr(WARP_K_STEPS):
                    for ii in range_constexpr(WARP_N_STEPS):
                        ...
                        n_idx = b_row_base + n_offset + warp_atom_n_idx + ldmatrix_b_n_idx
            ...
            if const_expr(IS_SPLIT_K):
                ...
                for i in range_constexpr(LDG_REG_C_COUNT):
                    ...
                    cond_boundary = arith.cmpi(
                        arith.CmpIPredicate.ult, m_global_idx, seq_len
                    )
                    ...
                    with ir.InsertionPoint(cond_boundary_if.then_block):
                        ...
                        for vec_idx in range_constexpr(LDG_VEC_SIZE // 2):
                            ...
                            pair_ptr_v = get_llvm_ptr(
                                C, linear_offset_c + vec_idx * 2, DTYPE_BYTES
                            )
                            llvm.AtomicRMWOp(
    ...
    def launch_jdbba_kernel(
        ...
        jdbba_kernel._func.__name__ = KERNEL_NAME
        launcher = jdbba_kernel(
            C, A, B, BIAS, seq_offsets, max_seq_len, batch_count, semaphore, signal
        )
        ...
        launcher.launch(
            grid=(N_BLOCKS, m_tiles, batch_count),
            block=(BLOCK_THREADS, 1, 1),
            stream=stream,
        )

    return launch_jdbba_kernel
