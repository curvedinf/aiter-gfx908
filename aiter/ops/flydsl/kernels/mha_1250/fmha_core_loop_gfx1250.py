...
def _get_valu_slots(stage, gemm_idx, cycle23, default=2,
                    qpoint=GEMM_INST_COUNT // 4 - 1):
...
def _build_softmax_part2_ops(ty, msb, blk, sp_pairs, ss, sgpr,
                             skip_rescale_sum=False,
                             sp_lo_cache=None, sp_hi_cache=None):
...
def _build_all_softmax_part2_ops(ty, blk, sp_pairs_all,
...
def _build_softmax_part0_ops(ty, msb, sp_pairs, ss, sgpr):
    ...
    def op_cur_max(b=bank):
        ss['local_max'][b] = _atom_max3_num_f32(
...
def _build_softmax_part1_ops(ty, ss, sgpr):
    ...
    def op_max01():
        ss['local_max'][0] = _atom_max3_num_f32(
            ss['local_max'][0], ss['local_max'][1],
            ss['pre_max_log2e_scl'][0], 0)
    ops.append(op_max01)
    msb_assign.append(0)
    ...
    def op_max23():
        ss['local_max'][2] = _atom_max3_num_f32(
            ss['local_max'][2], ss['local_max'][3],
            ss['pre_max_log2e_scl'][2], 2)
    ops.append(op_max23)
    msb_assign.append(2)
...
def _build_all_softmax_gemm2_ops(ty, blk, sp_pairs_all,
                                 softmax_state, sgpr_state,
                                 skip_rescale_sum=False):
    ...
    for m in range_constexpr(NUM_MSB):
        sp_pairs = sp_pairs_all[m]
        p0_ops = _build_softmax_part0_ops(
            ty, m, sp_pairs, softmax_state, sgpr_state)
        ops_by_rid[m] = p0_ops

    # PART1: 8 ops (cross-MSB)
    p1_ops, p1_msb_assign = _build_softmax_part1_ops(
        ty, softmax_state, sgpr_state)
    ops_by_rid[4] = p1_ops
...
def _cl_su_v3_stage(
    ty, stage,
    gemm_blk, gemm_su,
    tdm_type, tdm_blk, tdm_su,
    lds_type, lds_blk, lds_su,
    q_tiles, kv_tiles, sp_tiles,
    kv_lds_addrs, kv_tiles_next,
...
def _core_loop_gemm1(
    ...
    sp_out, kv_out, _, _ = _core_loop(
        ...
        kv_lds_addrs, tdm_state, softmax_state, sgpr_state,
        gemm2=False,
    )
...
def _softmax_part2_only(ty, blk, sp_pairs_all, softmax_state, sgpr_state,
                        skip_rescale_sum=False):
...
def _build_p_tiles_from_softmax(ty, softmax_state):
    ...
    for su in range_constexpr(CNT_SU):
        ...
        for m_tile in range_constexpr(2):
            ...
            combined = (p_bf16_all[msb_lo][p_start:p_start + 4] +
                        p_bf16_all[msb_hi][p_start:p_start + 4])
            su_tiles.append(
                _pack_v2bf16_to_v16bf16(ty, combined, m_tile))
...
def _core_loop(
    ...
    stage_configs = [
        (0, KV_V if memload else KV_NONE,
         KV_K, blk, 1),
        (1, KV_V if memload else KV_NONE,
         KV_K, blk, 2),
        (2, KV_NONE,
         KV_K, blk, 3),
        (3, KV_NONE,
    ...
    for stage_idx, (g_su, t_type, l_type, l_blk, l_su) in \
        ...
        kv_tiles_next_raw = [
            [None] * _n_lds for _ in range_constexpr(NUM_MSB)]
    ...
    for stage_idx, (g_su, l_type, l_blk, l_su) in \
