...
def fmha_fwd_kernel(
    ...
    q_frags_raw = _phase4_q_load_flydsl(
        ...
        q_tile_offset_bytes=_raw(q_offset))
    rocdl.sched_barrier(0)
    ...
    def _ep_finish(o_tiles, sp_pairs_in, exp_delta_rescale, v_base_for_pv,
        ...
        # BSHD-safe O byte offset:
        #   _o_tok = bx*128 + wave*32         (token index within batch)
        #   elem_off = bz*stride_o_batch + by*stride_o_head + _o_tok*stride_o_seq
        #   byte_off = elem_off * BPP(=2)
        # Note: stride_o_{seq,head,batch} are passed in ELEMENT units (test_acc uses *_e).
        _o_tok = _sa2.AddIOp(
        ...
        _o_elem_off = _sa2.AddIOp(
            _sa2.AddIOp(
                _sa2.MulIOp(bz, stride_o_batch).result,
