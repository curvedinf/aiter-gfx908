...
Vendored from FlyDSL (FlyDSL/kernels/pipeline_utils.py). Kept here because
the gfx1250 mxscale MoE kernels under
``aiter/aiter/ops/flydsl/kernels/moe_gemm_2stage_common_gfx1250.py`` use a
bare ``from kernels.pipeline_utils import make_tail_plan`` import that
resolves through ``aiter.fused_moe._ensure_flydsl_kernels_path`` (which
adds ``aiter/aiter/ops/flydsl/`` to ``sys.path`` so ``kernels.*`` resolves
to this directory). Without this file, the multi-buffer pipeline path
(``num_buffers >= 2``) raises ModuleNotFoundError when invoked through
aiter's ``fused_moe`` entrypoint.
"""
