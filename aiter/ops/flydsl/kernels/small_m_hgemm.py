    ...
    *,
    ...
    if SMEM_USE > MAX_LDS_BYTES:
        raise RuntimeError(
            ...
            f"but device limit is {MAX_LDS_BYTES} bytes "
            f"(arch={GPU_ARCH}, TILE_M={TILE_M}, TILE_N={TILE_N}, TILE_K={TILE_K}, "
            f"SPLIT_K={SPLIT_K}, B_TO_LDS={B_TO_LDS}, N_TILE_REPEAT={N_TILE_REPEAT}, "
            f"PERSISTENT_N_TILES={PERSISTENT_N_TILES})",
        )
    ...
    def small_m_hgemm_kernel(
        ...
        def prepare_split_k_tile(tile_n_offset, tile_signal_idx):
            ...
            cond_first_if = scf.IfOp(cond_first, results_=[], has_else=False)
        ...
        if const_expr(B_TO_LDS):
            ...
            def run_b_to_lds_tile(
                ...
                if const_expr(IS_SPLIT_K):
                    prepare_split_k_tile(tile_n_offset, tile_signal_idx)
