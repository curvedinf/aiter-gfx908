...
def _get_compiled_silu_fused(
    ...
    return build_silu_and_mul_fq_module(
        inter_dim, topk, quant_mode, gui_layout,
        act=act, enable_bias=enable_bias,
    )
...
def flydsl_moe_stage1(
    ...
    *,
    ...
    # split-K: bias is applied in the post-activation kernel (silu_and_mul_fq /
    # silu_and_mul), not the GEMM itself; pass an empty tensor to the GEMM.
    ...
    # Build topk_ids_arg and bias_arg for the post-activation kernel.
    # split-K bias is indexed by expert_id (per topk slot), not sorted_token_id.
    use_splitk_bias = _is_splitk and bias is not None
    if use_splitk_bias and topk_ids is None:
        raise ValueError("topk_ids is required for split-K FlyDSL stage1 bias")
    topk_ids_arg = (
    ...
    if _gui_sk_fused:
        ...
        _silu_fused_k = _get_compiled_silu_fused(
            ...
            act=act, enable_bias=use_splitk_bias,
        )
        _run_compiled(
            _silu_fused_k,
            (
