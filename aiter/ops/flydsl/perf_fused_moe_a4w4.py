...
def dequant_sorted_fp4(
    ...
    *,
    ...
    padded_rows = max(sorted_ids.numel(), sorted_scale.numel() // (out_dim // 32))
    padded_rows = (padded_rows + tile_m - 1) // tile_m * tile_m
    scale_groups = out_dim // 32
    scale_f32 = _e8m0_to_f32(
        sorted_scale.view(torch.uint8)[: padded_rows * scale_groups].view(
            padded_rows, scale_groups
        )
    )
    out = torch.zeros(
        (token_num, topk, scale_groups), dtype=torch.float32, device=fp4_packed.device
    )
    ...
    return (a_f32.view(token_num, topk, scale_groups, 32) * out.unsqueeze(-1)).view(
        token_num, topk, out_dim
    )
...
def parse_stage1_kernel_name(name: str) -> dict:
    ...
    """
    params = dict(
        tile_n=128, tile_k=256, waves_per_eu=3, k_batch=1, b_nt=2, gate_only=False
    )
...
def setup_data(token, model_dim, inter_dim, E, topk, block_m, dtype=torch.bfloat16):
    ...
    fused_out = torch.zeros(
        (token, topk, inter_dim // 2), dtype=torch.uint8, device="cuda"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fused vs Non-fused MoE A4W4 perf comparison"
    )
    parser.add_argument(
        "-t",
        "--tokens",
    ...
    parser.add_argument(
        "--csv", type=str, default=CSV_PATH, help="Path to tuned CSV config"
    )
    ...
    for c in cases:
        ...
        print(
            f"  params: tile_n={kp['tile_n']} waves_per_eu={kp['waves_per_eu']} "
        ...
        print("      running non-fused stage1...", flush=True)
        ...
        print("      running fused stage1...", flush=True)
        ...
        print("      running non-fused stage1+quant...", flush=True)
        ...
        print("      running fused stage1 (with sort)...", flush=True)
        ...
        print("      running non-fused e2e...", flush=True)
        ...
        print("      running fused e2e...", flush=True)
        ...
        prec_nf = (
            "PASS"
            if err_nf_vs_torch == 0
            else ("WARN" if err_nf_vs_torch <= 0.05 else "FAIL")
        )
        ...
        prec_f = (
            "PASS"
            if err_f_vs_torch == 0
            else ("WARN" if err_f_vs_torch <= 0.05 else "FAIL")
        )
        ...
        prec_nf_f = (
            "PASS" if err_nf_vs_f == 0 else ("WARN" if err_nf_vs_f <= 0.05 else "FAIL")
        )
    ...
    print(f"{'='*120}")
    print(
        f"  {'token':>6s}  {'idim':>5s}  {'nf_s1':>8s}  {'f_s1':>8s}  "
