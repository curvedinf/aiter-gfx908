...
# Try dsl2 source first (editable install), then vendored copy
import os as _os
import sys as _sys
...
try:
    ...
    for _ in range(4):
        ...
        _kdir = _os.path.join(_cur, "kernels", "fused_rope_cache_kernel.py")
        if _os.path.isfile(_kdir):
...
def flydsl_fused_qk_rope_reshape_and_cache(
    ...
    # -- Fallback conditions --
    # FlyDSL kernel has soffset codegen bugs for certain QH/KH combos with T>1:
    #   QH=64 (WARP_SIZE), QH==KH (e.g. MHA), KH=1 with large QH_per_KH
    if t > 1 and (qh == 64 or qh == kh or (kh == 1 and qh > 8) or d == 64):
        _LOGGER.debug("FlyDSL RoPE: unsupported QH/KH combo with T>1, falling back to Triton")
    ...
    # FlyDSL kernel always uses half-dim cos/sin (reuse_freqs_front_part=True).
    cos_dim = cos.shape[-1]
    if cos_dim != d // 2:
        _LOGGER.debug("FlyDSL RoPE: full-dim cos/sin not supported, falling back to Triton")
        return _triton_fallback(
            q, k, v, key_cache, value_cache, slot_mapping, pos,
            cos, sin, k_scale, v_scale, is_neox, flash_layout,
            apply_scale, offs, q_out, k_out, output_zeros, zeros_out,
    ...
    if not flash_layout:
        _LOGGER.debug("FlyDSL RoPE: non-flash layout not supported, falling back to Triton")
        return _triton_fallback(
            q, k, v, key_cache, value_cache, slot_mapping, pos,
            cos, sin, k_scale, v_scale, is_neox, flash_layout,
            apply_scale, offs, q_out, k_out, output_zeros, zeros_out,
    ...
    slot_i32 = slot_mapping.to(torch.int32) if slot_mapping.dtype != torch.int32 else slot_mapping
