...
def get_gemm_config(
    ...
    This function provides a unified way to load GEMM configs for gluon kernels
    (gfx1250 only). It uses the following logic:
    1. Load default config file: gfx1250-{config_name}.json
    2. If N and K are provided, try to load specialized config:
       gfx1250-{config_name}-N={N}-K={K}.json
       Or if specialized_filename is provided, use:
       gfx1250-{config_name}-{specialized_filename}.json
    ...
    cache_key = f"{_ARCH}_{config_name}"
    ...
    if specialized_filename is not None:
        ...
        if spec_key not in get_gemm_config._config_cache[cache_key]:
            fpath = f"{AITER_GLUON_CONFIGS_PATH}/gemm/{_ARCH}-{config_name}-{specialized_filename}.json"
