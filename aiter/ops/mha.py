...
def cmdGenFunc_mha_fwd(
    ...
    q_descale_scalar: float,
    k_descale_scalar: float,
    v_descale_scalar: float,
...
def cmdGenFunc_mha_varlen_fwd(
    ...
    if block_table is None:
        ...
        if dropout_p == 0:
            ...
            md_name += "_dropout"
            filter_fwd += "_dropout*"
        # TODO - squant and nsquant (wait for ck rename)
        if min_seqlen_q == 0:
            md_name += "_nskip"
            filter_fwd += "_nskip*"
        else:
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        out_padded, softmax_lse, S_dmask, rng_state = _flash_attn_forward(
            ...
            q_descale_scalar=1.0,
            k_descale_scalar=1.0,
            v_descale_scalar=1.0,
            bias=bias,
...
def flash_attn_fp8_pertensor_func(
    ...
    out_padded, _, _, _ = _flash_attn_forward(
        ...
        window_size_left=int(window_size[0]),
        window_size_right=int(window_size[1]),
        q_descale_scalar=q_descale,
        k_descale_scalar=k_descale,
        v_descale_scalar=v_descale,
        bias=None,
        alibi_slopes=None,
        return_lse=False,
