...
_gemm_a16w16_basic_pipelined_v2_repr = make_kernel_repr(
    "_gemm_a16w16_gfx1250_basic_pipelined_v2_kernel", _GLUON_REPR_KEYS
)

_gemm_a16w16_finer_interleaved_pipelined_v2_repr = make_kernel_repr(
    "_gemm_a16w16_gfx1250_finer_interleaved_pipelined_v2_kernel", _GLUON_REPR_KEYS
)
...
@gluon.jit(repr=_gemm_a16w16_basic_pipelined_v2_repr)
def _gemm_a16w16_basic_pipelined_v2_kernel(
    a_ptr,
    b_ptr,
    c_ptr,
    bias_ptr,
    ...
    """Pipelined v2: mfma first, then TDM, then wait + ds_load.

    Generalized for any NUM_BUFFERS >= 2.  Fills NUM_BUFFERS-1 tiles via TDM,
    pre-loads tile 0 into registers, then runs:
    loop { wmma n, TDM next, wait, ds_load next },
    epilogue drains remaining tiles.
    """
    USE_L2_PREFETCH: gl.constexpr = L2_PREFETCH_DISTANCE > 0
    gl.static_assert(NUM_BUFFERS >= 2, "basic_pipelined_v2 requires NUM_BUFFERS >= 2")
    ...
    # ── TDM prologue: fill pipeline with NUM_BUFFERS-1 tiles ──
    for _ in gl.static_range(NUM_BUFFERS - 1):
        if PHYSICAL_MK:
            gl.amd.gfx1250.tdm.async_load(
                a_desc,
    ...
    for _ in range(num_k_tiles - (NUM_BUFFERS - 1)):
        ...
        # ds_load next tile into registers
        if PHYSICAL_MK:
            next_a = gl.amd.cdna4.async_copy.load_shared_relaxed(
...
@gluon.jit(repr=_gemm_a16w16_finer_interleaved_pipelined_v2_repr)
def _gemm_a16w16_finer_interleaved_pipelined_v2_kernel(
    a_ptr,
    b_ptr,
    c_ptr,
    bias_ptr,
    ...
    """Finer interleaved pipelined v2: mfma issued before the independent ds_load.

    TDM 0, TDM 1, wait 0, ds_load 0 half1,
    loop { mfma n half1, ds_load n half2, TDM n+2,
           wait n+1, mfma n half2, ds_load n+1 half1 },
    epilogue.
    Requires NUM_BUFFERS >= 3 and BLOCK_K >= 64.
    """
    USE_L2_PREFETCH: gl.constexpr = L2_PREFETCH_DISTANCE > 0
    HALF_K: gl.constexpr = BLOCK_K // 2
    gl.static_assert(NUM_BUFFERS >= 3, "finer_interleaved_pipelined_v2 requires NUM_BUFFERS >= 3")
    gl.static_assert(HALF_K >= 32, "finer_interleaved_pipelined_v2 requires BLOCK_K >= 64")
...
_KERNEL_MAP = {
    ...
    "basic_pipelined_v2": _gemm_a16w16_basic_pipelined_v2_kernel,
    ...
    "finer_interleaved_pipelined_v2": _gemm_a16w16_finer_interleaved_pipelined_v2_kernel,
}
...
def gemm_a16w16_gfx1250(
    ...
    Args:
        ...
        kernel_type: Kernel variant to use:
            ...
            - "basic_pipelined_v2": mfma first, then TDM + wait + ds_load (requires NUM_BUFFERS >= 2)
            - "finer_interleaved_pipelined_v2": mfma before independent ds_load (requires NUM_BUFFERS >= 3, BLOCK_K >= 64)
