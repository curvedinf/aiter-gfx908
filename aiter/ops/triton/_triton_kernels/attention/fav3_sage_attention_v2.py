...
from aiter.ops.triton.utils._triton.hadamard_rotation import apply_hadamard_rotation_qk
...
def smooth_q_kernel(
    ...
    x = x - x_mean[None, :]
    x = x.to(q_output_ptrs.dtype.element_ty)
    tl.store(q_output_ptrs, x, mask=mask)
...
def sage_quant(
    ...
    if smooth_q:
        ...
        q = q_smooth
        delta_s = torch.ones((b, h_kv, Q_NUM_BLKS, kv_len), device=q.device, dtype=torch.float32)
        # add compute of delta_s
    else:
        q = q * sm_scale
        delta_s = None
...
def v_quant_delta_s_kernel(
    ...
    if pid < q_task_count:
        # here we do delta S computation (q_mean * k)
        off_blk, off_h, off_b = pid_grid_3d(pid, Q_NUM_BLKS, Q_HEAD, BATCH)
        offs_q_mean = off_blk * BLK_Q + offs_blk_q

        q_offs = (
            off_b * stride_qz
            + off_h * stride_qh
            + offs_qn[:, None] * stride_qmn
            + offs_d[None, :]
        )
        ...
        delta_s_output_ptrs = K_Output + v_offs
