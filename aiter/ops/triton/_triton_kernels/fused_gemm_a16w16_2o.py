...
@triton.jit
def _fused_gemm_a16_w16_2o_kernel(
    a_ptr,
    b_ptr,
    c_ptr,
    c_no_act_ptr,
    ...
    if split_k_start < K:
        ...
        if use_activation and NUM_KSPLIT == 1:
            c_no_act = accumulator.to(c_no_act_ptr.type.element_ty)
            c_no_act_ptrs = (
                c_no_act_ptr
            ...
            tl.store(c_no_act_ptrs, c_no_act, mask=c_mask)
...
@triton.jit
def _fused_gemm_a16w16_2o_reduce_kernel(
    c_in_ptr,
    c_out_ptr,
    c_no_act_out_ptr,
    M,
    ...
    if use_activation:
        c_no_act = c.to(c_no_act_out_ptr.type.element_ty)
        c_no_act_out_ptrs = (
            c_no_act_out_ptr
            + (offs_m[:, None] * stride_c_out_m)
            + (offs_n[None, :] * stride_c_out_n)
        )
        tl.store(c_no_act_out_ptrs, c_no_act)
...
def _get_config(
    ...
    if not hasattr(_get_config, "_config_dict"):
        ...
        fpath = f"{AITER_TRITON_CONFIGS_PATH}/gemm/MI350X-FUSED-GEMM-A16W16-2O.json"
    ...
    if key not in _get_config._config_dict.keys():
        dev = arch_info.get_device()
        fpath = f"{AITER_TRITON_CONFIGS_PATH}/gemm/MI350X-FUSED-GEMM-A16W16-2O-N={N}-K={K}.json"
