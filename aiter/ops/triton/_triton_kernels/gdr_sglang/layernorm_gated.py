...
# 从 sglang.srt.utils 移植的完整实现
from contextlib import contextmanager
from functools import lru_cache

@lru_cache(maxsize=1)
def is_cpu() -> bool:
    """Check if CPU is the only available device"""
    return not torch.cuda.is_available() and not hasattr(torch, "npu")
...
def device_context(device: torch.device):
    """Context manager for device selection, supporting CUDA/CPU/NPU/XPU"""
