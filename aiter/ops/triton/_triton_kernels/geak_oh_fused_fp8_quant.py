...
def _fp8_quant_op(
    ...
    # Reshape to 2D instead of 3D to avoid Triton max reduction bug
    x = x.reshape(NUM_QUANT_BLOCKS, QUANT_BLOCK_SIZE)
    
    # Create mask - compute valid elements per group
    group_idx = tl.arange(0, NUM_QUANT_BLOCKS)[:, None]
    elem_idx = tl.arange(0, QUANT_BLOCK_SIZE)[None, :]
    valid_mask = (group_idx * QUANT_BLOCK_SIZE + elem_idx) < n_cols
    
    # Compute abs, mask, and find max per group
    x_abs_masked = tl.where(valid_mask, tl.abs(x), 0.0)
    m = tl.maximum(tl.max(x_abs_masked, axis=-1), 1e-10)
    
    # Compute scale and quantize in one expression
    scale_recip = (DTYPE_MAX / m)[:, None]
    x_quant = tl.clamp(x * scale_recip, DTYPE_MIN, DTYPE_MAX)
    scale_out = (m / DTYPE_MAX).to(tl.float32)

    return x_quant, scale_out
...
def _fused_rms_fp8_group_quant_kernel(
    ...
    out1_fp8, out1_block_scales = _fp8_quant_op(
        norm1, inp1_n_cols, 1, BLOCK_SIZE_N, QUANT_BLOCK_SIZE, DTYPE_MAX, DTYPE_MIN
    )

    # Store quantized output
    tl.store(
        out1_fp8_ptr + m_pid * out1_fp8_row_stride + n_offs * out1_fp8_col_stride,
        tl.ravel(out1_fp8).to(out1_fp8_ptr.dtype.element_ty),
        mask=mask1,
    )
    ...
    tl.store(
        ...
        tl.ravel(out1_block_scales).to(out1_bs_ptr.dtype.element_ty),
...
def _fused_flatten_fp8_group_quant_kernel(
    ...
    out, out_block_scales = _fp8_quant_op(
        x, N2, 1, BLOCK_SIZE_N2, QUANT_BLOCK_SIZE, DTYPE_MAX, DTYPE_MIN
    )
    out = tl.ravel(out)
...
def _fused_reduce_act_mul_fp8_group_quant(
    ...
    y, y_scale = _fp8_quant_op(
        x, N1, 1, BLOCK_SIZE_N1, QUANT_BLOCK_SIZE, DTYPE_MAX, DTYPE_MIN
    )
    y = tl.ravel(y)
