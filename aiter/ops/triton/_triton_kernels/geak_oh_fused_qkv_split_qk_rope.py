...
@triton.jit
def _get_neox_rotated_x_optimized(
    x,
    x_rotated_mask,
...
@triton.jit
def _get_gptj_rotated_x_optimized(
    x,
    x_rotated_mask,
    BLOCK_T: tl.constexpr,
    BLOCK_D: tl.constexpr,
...
def _fused_qkv_split_qk_rope_kernel(
    ...
    if HAVE_POS:
        t_pos_offs = t_offs * stride_pos_t
        t_pos = tl.load(pos_ptr + t_pos_offs, mask=t_mask, other=0)
        if HAVE_OFFS:
            t_off = tl.load(off_ptr + t_pos_offs, mask=t_mask, other=0)
            t_cos_offs = t_pos + t_off
        else:
            t_cos_offs = t_pos
    else:
        t_cos_offs = t_offs

    if HAVE_NOPE and not REUSE_FREQS_FRONT_PART:
        d_cos_offs = d_offs % BLOCK_D_HALF
    ...
    # Load cos and sin together for better memory coalescing
    cos = tl.load(cos_ptr + cos_offs, mask=cos_mask, cache_modifier=".cg")
    sin = tl.load(sin_ptr + cos_offs, mask=cos_mask, cache_modifier=".cg")
    ...
    q = tl.load(qkv_ptr + q_in_offs, mask=x_mask, cache_modifier=".ca")

    if IS_NEOX:
        q_rotated = _get_neox_rotated_x_optimized(
            q, qk_rotated_mask, BLOCK_T, BLOCK_D, BLOCK_D_HALF
        )
    else:
        q_rotated = _get_gptj_rotated_x_optimized(
            q, qk_rotated_mask, BLOCK_T, BLOCK_D, BLOCK_D_HALF
        )
    ...
    if hq < KVH:
        ...
        k = tl.load(qkv_ptr + k_in_offs, mask=x_mask, cache_modifier=".ca")
        v = tl.load(qkv_ptr + v_in_offs, mask=x_mask, cache_modifier=".ca")

        if IS_NEOX:
            k_rotated = _get_neox_rotated_x_optimized(
                k, qk_rotated_mask, BLOCK_T, BLOCK_D, BLOCK_D_HALF
            )
        else:
            k_rotated = _get_gptj_rotated_x_optimized(
                k, qk_rotated_mask, BLOCK_T, BLOCK_D, BLOCK_D_HALF
            )
