...
def _fused_moe_kernel_mxfp4(
    ...
    # Precompute offs_token // top_k for reuse in multiple places
    offs_token_div_topk = offs_token // top_k
    ...
    if is_a_microscaled_format:
        ...
        if SWIZZLE_MX_A:
            ...
            # Reuse offs_token_div_topk computed earlier
            a_mx_scale_ptrs = (
                a_mx_scale_ptr
                + offs_scale_ak.to(tl.int64)[None, :] * stride_amxk
                + offs_token_div_topk.to(tl.int64)[:, None] * stride_amxm
            )
    else:
    ...
    # Precompute base offsets to reduce pointer arithmetic (reuse offs_token_div_topk from above)
    a_base_offset = offs_token_div_topk[:, None] * stride_am
    b_base_offset = off_expert * stride_be
    
    a_ptrs = a_ptr + a_base_offset + offs_a_k[None, :] * stride_ak
    b_ptrs = b_ptr + b_base_offset + (offs_b_k[:, None] * stride_bk + offs_b_n[None, :] * stride_bn)
    ...
    # Precompute loop bounds and strides to reduce overhead
    num_k_blocks = tl.cdiv(K, PACKED_BLOCK_K_A)
    a_stride_k = PACKED_BLOCK_K_A * stride_ak
    b_stride_k = PACKED_BLOCK_K_B * stride_bk
    
    # Precompute scale strides outside loop
    if is_a_microscaled_format:
        if SWIZZLE_MX_A:
            a_mx_scale_stride = MX_SCALE_BLOCK_K_A // 4 * stride_amxk
        else:
            a_mx_scale_stride = MX_SCALE_BLOCK_K_A * stride_amxk
    if is_b_microscaled_format:
        if SWIZZLE_MX_B:
            b_mx_scale_stride = MX_SCALE_BLOCK_K_B // 4 * 512
        else:
            b_mx_scale_stride = MX_SCALE_BLOCK_K_B * stride_bmxk
    
    # Precompute token mask for A loads
    token_mask_2d = token_mask[:, None]
    
    # Add compiler hints for better optimization
    tl.assume(num_k_blocks > 0)
    
    for k in range(0, num_k_blocks):
        # Load the next block of A and B
        # Use eviction policies: A is reused across N blocks, B is reused across M blocks
        if EVEN_K:
            a = tl.load(a_ptrs, mask=token_mask_2d, other=0.0, eviction_policy="evict_last")
            b = tl.load(b_ptrs, eviction_policy="evict_last")
        else:
            # Compute k_offset once and reuse
            k_offset_a = k * PACKED_BLOCK_K_A
            k_offset_b = k * PACKED_BLOCK_K_B
            # Compute K bounds once
            k_bound_a = K - k_offset_a
            k_bound_b = K - k_offset_b
            a_mask = token_mask_2d & (offs_a_k[None, :] < k_bound_a)
            b_mask = offs_b_k[:, None] < k_bound_b
        ...
        # Load microscale factors and compute dot product
        if is_a_microscaled_format or is_b_microscaled_format:
            if is_a_microscaled_format:
                if EVEN_K:
                    a_mx_scales = tl.load(a_mx_scale_ptrs, eviction_policy="evict_last")
                else:
                    # Reuse k_bound_a from above and precompute divisor
                    scale_bound_a = k_bound_a // (MX_PACK_DIVISOR // A_PACK_DIVISOR)
                    mask_ak_scale = offs_scale_ak < scale_bound_a
                    a_mx_scales = tl.load(a_mx_scale_ptrs, mask=mask_ak_scale[None, :], other=0.0, eviction_policy="evict_last")
            else:
                a_mx_scales = None
            
            if EVEN_K:
                b_mx_scales = tl.load(b_mx_scale_ptrs, eviction_policy="evict_last")
            else:
                # Reuse k_bound_b from above and precompute divisor
                scale_bound_b = k_bound_b // (MX_PACK_DIVISOR // B_PACK_DIVISOR)
                mask_bk_scale = offs_scale_bk < scale_bound_b
                b_mx_scales = tl.load(b_mx_scale_ptrs, mask=mask_bk_scale[None, :], other=0.0, eviction_policy="evict_last")
            ...
            # Advance scale pointers
            if is_a_microscaled_format:
                a_mx_scale_ptrs += a_mx_scale_stride
            if is_b_microscaled_format:
                b_mx_scale_ptrs += b_mx_scale_stride
        
        # Advance data pointers
        a_ptrs += a_stride_k
        b_ptrs += b_stride_k

    # Multiply with the scalar weight (fuse operations)
    scale_factor = a_scale * b_scale
    if MUL_ROUTED_WEIGHT:
        moe_weight = tl.load(topk_weights_ptr + offs_token, mask=token_mask, other=0)
        # Fuse scaling operations: accumulator * (a_scale * b_scale * moe_weight)
        accumulator = accumulator * (scale_factor * moe_weight[:, None])
    else:
