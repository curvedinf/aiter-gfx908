...
def _dynamic_mxfp4_quant_kernel(
    ...
    # GEAK OPTIMIZATION: Hoist loop-invariant M offset computations
    # This avoids redundant computation of M-dimension offsets in each loop iteration
    offs_m = tl.arange(0, BLOCK_SIZE_M)
    base_m = pid_m * BLOCK_SIZE_M
    x_offs_m = base_m + offs_m
    x_offs_m_scaled = x_offs_m[:, None] * stride_x_m
    out_offs_m_scaled = x_offs_m[:, None] * stride_x_fp4_m
    bs_offs_m_scaled = x_offs_m[:, None] * stride_bs_m
    m_mask = x_offs_m < M

    for pid_n in tl.range(start_n, min(start_n + NUM_ITER, N), num_stages=NUM_STAGES):
        x_offs_n = pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
        x_offs = x_offs_m_scaled + x_offs_n[None, :] * stride_x_n

        if EVEN_M_N:
            # GEAK OPTIMIZATION: Use cache_modifier=".cg" for global cache hint
            x = tl.load(x_ptr + x_offs, cache_modifier=".cg").to(tl.float32)
        else:
            x_mask = m_mask[:, None] & (x_offs_n < N)[None, :]
        ...
        out_offs = out_offs_m_scaled + out_offs_n[None, :] * stride_x_fp4_n

        if EVEN_M_N:
            # GEAK OPTIMIZATION: Use cache_modifier=".cs" for streaming store
            tl.store(x_fp4_ptr + out_offs, out_tensor, cache_modifier=".cs")
        else:
            out_mask = m_mask[:, None] & (out_offs_n < (N // 2))[None, :]
            tl.store(x_fp4_ptr + out_offs, out_tensor, mask=out_mask, cache_modifier=".cs")

        bs_offs_n = pid_n * NUM_QUANT_BLOCKS + tl.arange(0, NUM_QUANT_BLOCKS)
        bs_offs = bs_offs_m_scaled + bs_offs_n[None, :] * stride_bs_n
        if EVEN_M_N:
            # GEAK OPTIMIZATION: Use cache_modifier=".cs" for streaming store
