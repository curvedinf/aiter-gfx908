...
# 1-STAGE KERNEL (tiny rows) - OPTIMIZED
@triton.jit
def _topk_kernel(
    X,
    OUT_V,
    ...
    # Optimized: use static_range for better unrolling
    for j in core.static_range(0, K):
        vmax = tl.max(vals, axis=0)
        # Optimized: combine operations to reduce intermediate values
        eq_mask = vals == vmax
        # Use a large constant instead of creating zeros_like
        big_val = tl.full([BLOCK], BLOCK, dtype=tl.int64)
        masked_idxs = tl.where(eq_mask, idxs, big_val)
        arg = tl.min(masked_idxs, axis=0)
...
# 2-STAGE KERNEL (large rows) - OPTIMIZED STAGE 1
@triton.jit
def topk_stage1_kernel(
    ...
    # Optimized: Load once and convert to float32
    x_val = tl.load(x_ptr + cols, mask=mask, other=FILL_VALUE).to(tl.float32)
    
    # Optimized: Use static_range for better loop unrolling when k is small
    for k_idx in range(k):
        ...
        # Optimized: Simplified masking operation
        x_val = tl.where(cols == chunk_select_idx, FILL_VALUE, x_val)
