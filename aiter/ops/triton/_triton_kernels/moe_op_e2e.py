...
def e2e_moe_kernel(
    ...
    - W1: The first layer weight tensor with shape (E, N * 2, K).
    - W2: The second layer weight tensor with shape (E, K, N).
        N represents the size of the intermediate token after the gated activation.
    ...
    # offset for silu_acc
    i0 = (pid_n * BLOCK_SIZE_N + offs_i0) % N
    # offset for mul_acc
    i1 = (pid_n * BLOCK_SIZE_N + offs_i1) % (N * 2)
    ...
    silu_acc = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.float32)
    mul_acc = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.float32)
...
def e2e_moe_kernel_fp8(
    ...
    # gated activation
    scale_matrix = a_scale * w1_scale
    silu_acc = _silu_exp2(silu_acc * scale_matrix)
    acc = silu_acc * mul_acc * scale_matrix
...
@triton.jit
def e2e_moe_kernel_fp8_blockscale(
    A,
    W1,
    W2,
    Out,
    ...
    num_scales_along_n: tl.constexpr = (N + group_n - 1) // group_n
    num_scales_along_bn: tl.constexpr = (BLOCK_SIZE_N + group_n - 1) // group_n
    num_scales_along_bk2: tl.constexpr = (BLOCK_SIZE_K2 + group_k - 1) // group_k
    num_scales_along_bk1: tl.constexpr = (BLOCK_SIZE_K1 + group_k - 1) // group_k
    tl.static_assert(num_scales_along_bk1 == 1, "BLOCK_SIZE_K1 must be <= group_k")
    if PER_TOKEN_QUANT_A:
        tl.static_assert(
            group_k == K,
    ...
    # gated activation
    if not VARYING_K_SCALE:
        silu_acc = _silu_exp2(silu_acc * a_scale * w1_i0_scale)
        acc = silu_acc * mul_acc * a_scale * w1_i1_scale
    else:
        silu_acc = _silu_exp2(silu_acc)
        acc = silu_acc * mul_acc
    ...
    w2_scale_ptrs += (tl.arange(0, num_scales_along_bk2)[None, :]) * stride_w2sk
    # w2_scale_ptrs: (num_scales_along_bn, num_scales_along_bk2)
    ...
    for k2 in tl.range(0, num_k2, num_stages=1):
        ...
        )  # (num_scales_along_bn, num_scales_along_bk2)
        # only need to do descale before dot if inner dimension (n) spans multiple scale values
        if num_scales_along_bn > 1:
            w2_scale = repeat_interleave_triton(
                w2_scale, num_scales_along_bn, num_scales_along_bk2, group_n, 0
            )
            w2_scale = repeat_interleave_triton(
                w2_scale,
                num_scales_along_bn * group_n,
                num_scales_along_bk2,
                group_k,
                1,
            )
