...
@triton.jit
def _fused_silu_mxfp4_quant_moe_sort_kernel(
    x_ptr,
    x_fp4_ptr,
    sorted_ids_ptr,
    num_valid_ids_ptr,
    ...
    stride_x_fp4_m = tl.cast(stride_x_fp4_m, tl.int64)
    stride_x_fp4_n = tl.cast(stride_x_fp4_n, tl.int64)
    up_offset = tl.cast(Nx, tl.int64) * stride_x_n
    ...
    if pid < num_pid_x:
        ...
        x_offs = x_offs_m[:, None] * stride_x_m + x_offs_n[None, :] * stride_x_n
        x_mask = (x_offs_m < Mx)[:, None] & (x_offs_n < Nx)[None, :]
        gate = tl.load(x_ptr + x_offs, mask=x_mask).to(tl.float32)
        up = tl.load(x_ptr + x_offs + up_offset, mask=x_mask).to(tl.float32)
