...
import aiter
import triton
from aiter.ops.triton._triton_kernels.attention.fav3_sage_attention_v2 import (
    sage_quant,
    sage_fwd_v2,
    map_dims,
)
...
class _FAv3SageWrapperFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        q_fp4, q_descale, k_fp4, k_descale, v_fp8, v_descale, delta_s = sage_quant(
            q,
            k,
            v,
            fp8_dtype,
...
def fav3_sage_func(
    ...
    stride_qsz, stride_qsm, stride_qsh, _ = map_dims(
        q_descale.stride(), bshd_map
    )
    stride_ksz, stride_ksn, stride_ksh, _ = map_dims(
        k_descale.stride(), bshd_map
    )
