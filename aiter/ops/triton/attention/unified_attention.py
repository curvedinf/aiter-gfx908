...
from aiter.ops.triton._triton_kernels.attention.unified_attention import (
    UNIFIED_ATTN_AUTOTUNE,
    _AUTOTUNE_2D_PARAMS,
    _AUTOTUNE_3D_PARAMS,
    _AUTOTUNE_REDUCE_PARAMS,
    _2d_autotuner,
    _3d_autotuner,
    _reduce_autotuner,
...
def unified_attention(
    ...
    if _dispatch_2d:
        ...
        grid_2d = (num_kv_heads, total_num_q_blocks)

        global _2d_tune_cfg
        if UNIFIED_ATTN_AUTOTUNE and _2d_tune_cfg is None:
            _2d_kwargs = dict(
                output_ptr=out, query_ptr=q, key_cache_ptr=k,
                value_cache_ptr=v, sink_ptr=sinks,
                block_tables_ptr=block_table, seq_lens_ptr=seqused_k,
                alibi_slopes_ptr=alibi_slopes, qq_bias_ptr=qq_bias,
                scale=softmax_scale if softmax_scale is not None else 1.0,
                q_scale=q_descale, k_scale=k_descale, v_scale=v_descale,
                out_scale=1 / output_scale if output_scale is not None else 1.0,
                softcap=softcap, num_query_heads=num_query_heads,
                ...
                num_seqs=num_seqs, OUTPUT_FP8=output_scale is not None,
            ...
            config_filtered = {k: v for k, v in config.items()
                               if k not in _AUTOTUNE_2D_PARAMS}
            _2d_autotuner[grid_2d](**_2d_kwargs, **config_filtered)
            best = _2d_autotuner.best_config
            _2d_tune_cfg = dict(config)
            _2d_tune_cfg.update(best.kwargs)
            _2d_tune_cfg["num_warps"] = best.num_warps
            _2d_tune_cfg["num_stages"] = best.num_stages
        else:
            effective_config = _2d_tune_cfg if _2d_tune_cfg is not None else config
            kernel_unified_attention_2d[grid_2d](
                output_ptr=out,
                query_ptr=q,
                key_cache_ptr=k,
                value_cache_ptr=v,
        ...
        if UNIFIED_ATTN_AUTOTUNE and _3d_tune_cfg is None:
            ...
            _3d_kwargs = dict(
                ...
                key_cache_ptr=k, value_cache_ptr=v, sink_ptr=sinks,
                block_tables_ptr=block_table, seq_lens_ptr=seqused_k,
                alibi_slopes_ptr=alibi_slopes, qq_bias_ptr=qq_bias,
                scale=softmax_scale, q_scale=q_descale, k_scale=k_descale,
                softcap=softcap, num_query_heads=num_query_heads,
                num_queries_per_kv=num_queries_per_kv,
                block_table_stride=block_table_stride_0,
                query_stride_0=q.stride(0), query_stride_1=q.stride(1),
            ...
            _reduce_kwargs = dict(
                ...
                segm_output_ptr=segm_output, segm_max_ptr=segm_max,
                segm_expsum_ptr=segm_expsum, seq_lens_ptr=seqused_k,
            ...
            best_reduce = _reduce_autotuner.best_config
            _reduce_tune_cfg = dict(tuned_reduce)
            _reduce_tune_cfg["num_warps"] = best_reduce.num_warps
