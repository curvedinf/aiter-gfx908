...
class AiterCommunicator:
    ...
    def all_reduce(self, inp: torch.Tensor) -> torch.Tensor:
        ...
        # copy back to a fresh output. Matches CustomAllreduce API.
        # one_shot fuses start+end barriers into the kernel via monotonic
        # counters on the symmetric heap (never zeroed, graph-capturable).
        # async_op=True skips the host-side ctx.barrier(); iris omits it
        # unconditionally for one_shot anyway.
        assert self._shmem is not None
        try:
            ...
            if self._workspace is None:
                self._workspace = self._shmem.ccl.all_reduce_preamble(
                    input_buf, input_buf, config=self._config
                )
            self._workspace = self._shmem.ccl.all_reduce(
                input_buf, input_buf, workspace=self._workspace, config=self._config, async_op=True
            )
