...
__all__ = ["fused_allreduce_add_rms_row_quant_iris_twoshot"]
...
def fused_allreduce_add_rms_row_quant_iris_twoshot(
    input: torch.Tensor,
    rms_weight: torch.Tensor,
    rms_eps: float,
    ...
    """Fused AllReduce + Add + RMSNorm + per-row FP8 Quant (two-shot).

    Two-shot allreduce with inlined device barrier and fused
    RMSNorm + per-row FP8 quantization in a single kernel.
    scale_out shape is (M,) instead of (1,).
    """
    iris_mgr = get_iris_twoshot_row_manager()
    return iris_mgr.fused_allreduce_rmsnorm_row_quant(
