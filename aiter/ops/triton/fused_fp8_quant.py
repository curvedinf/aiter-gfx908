...
def _fp8_per_token_quant_op(
    ...
    m = tl.maximum(tl.max(tl.abs(x), axis=-1), 1e-10)
    scale_out = m.to(tl.float32) / DTYPE_MAX
    scale_recip = 1.0 / scale_out
    x = tl.clamp(x * scale_recip, DTYPE_MIN, DTYPE_MAX)
    
    return x, scale_out

@triton.jit
def _fused_rms_fp8_per_token_quant_kernel(
    inp1_ptr,
    weight1_ptr,
    inp2_ptr,
    weight2_ptr,
    ...
    FIRST_INPUT_RES: tl.constexpr,
    FIRST_INPUT_OUT: tl.constexpr,
    FISRT_INPUT_QUANT_OUT: tl.constexpr,
):
    m_pid = tl.program_id(0)
    n_offs = tl.arange(0, BLOCK_SIZE_N)
    ...
    if FISRT_INPUT_QUANT_OUT:
        out1_fp8, out1_block_scales = _fp8_per_token_quant_op(
            norm1, DTYPE_MAX, DTYPE_MIN
        )
        # store the results
        tl.store(
            out1_fp8_ptr + m_pid * out1_fp8_row_stride + n_offs * out1_fp8_col_stride,
        ...
        tl.store(
            out1_bs_ptr + m_pid * out1_bs_row_stride,
            out1_block_scales.to(out1_bs_ptr.dtype.element_ty)
        )
...
def fused_rms_fp8_per_token_quant(
    ...
    - out1_bs: The output matrix with shape (M, 1).
    - out2: The output matrix with shape (M, N2).
    - out_res1: The output matrix with shape (M, N1).
    ...
    out1_fp8 = None
    out1_bs = None
    out1_fp8_row_stride = 0
    out1_fp8_col_stride = 0
    out1_bs_row_stride = 0
    out1_bs_col_stride = 0
    DTYPE_MAX = 0
    if output_quantiezed_inp1:
        out1_fp8 = torch.empty((M, N1), dtype=dtype_quant, device=inp1.device)
        out1_bs = torch.empty((M, 1), dtype=torch.float32, device=inp1.device)
    ...
    _fused_rms_fp8_per_token_quant_kernel[(M, )](
        inp1,
        inp1_weight,
        inp2,
        inp2_weight,
        ...
        FIRST_INPUT_RES=(res1 is not None),
        FIRST_INPUT_OUT=output_unquantized_inp1,
        FISRT_INPUT_QUANT_OUT=output_quantiezed_inp1,
    )
