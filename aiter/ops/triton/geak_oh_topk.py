...
def one_stage_topk(
    ...
    # Optimized: Tune num_warps based on BLOCK size for better occupancy
    if BLOCK <= 256:
        num_warps = 4
    elif BLOCK <= 512:
        num_warps = 8
...
def two_stage_topk(x, k, dim=-1, largest=True):
    ...
    # Optimized: Better chunk size selection based on input size and k
    if topk_elem_cnt < 2048:
        chunk_size = 512
    elif topk_elem_cnt < 8192:
        chunk_size = 1024
    elif topk_elem_cnt < 32768:
        chunk_size = 2048
    else:
        chunk_size = 4096
    ...
    # Optimized: Tune num_warps for stage1 based on chunk_size
    if chunk_size <= 512:
        num_warps_s1 = 4
    ...
    # Optimized: Tune num_warps for stage2 based on BLOCK_SIZE
    if BLOCK_SIZE <= 256:
        num_warps_s2 = 4
