...
@gluon.jit
def gemm_tdm_pipelined_blockscale_kernel(a_ptr, b_ptr, c_ptr,  #
                              ...
                              GROUP_K: ttgl.constexpr):
    a_dtype: ttgl.constexpr = a_ptr.type.element_ty
    b_dtype: ttgl.constexpr = b_ptr.type.element_ty
    # gemm a8w8 -- TODO cast to fp8 instead
    # ttgl.static_assert(a_dtype.is_fp16(), "Only fp16 supported for A")
...
def test_runtime_gemm_tdm_pipelined(BLOCK_M, BLOCK_N, BLOCK_K, NUM_BUFFERS, TRANSPOSE_B, PERSISTENT, PREFETCH,
    ...
    # change these below ones a bit so its not a tensor just a pointer to a tensor or something
    a_scale = torch.rand([scale_m, scale_k], dtype=dtypes.fp16, device="cuda")
    b_scale = torch.rand([scale_n, scale_k], dtype=dtypes.fp16, device="cuda")
    stride_ascale_m, stride_ascale_k = a_scale.stride(0), a_scale.stride(1)
    stride_bscale_k, stride_bscale_n = (b_scale.stride(0), b_scale.stride(1)) if not TRANSPOSE_B else (b_scale.stride(1), b_scale.stride(0))
    ...
    if not PERSISTENT:
        ...
        #TODO
        kernel = gemm_tdm_pipelined_blockscale_kernel[grid](
