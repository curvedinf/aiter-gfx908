...
def _gemm_a8w8_blockscale_kernel(
    ...
    **scale_n = (N + GROUP_N - 1) // GROUP_N
    ...
    blocked_mk: gl.constexpr = gl.BlockedLayout(
        ...
        threads_per_warp=[8, 4], # 32 bc its 32 threads in a warp
    ...
    if (pid_k * SPLITK_BLOCK_SIZE) < K:
        ...
        for k in range(pid_k * num_k_iter, ((pid_k + 1) * num_k_iter) - 1):
            ...
            producer = issue_loads(producer, a_desc, b_desc, 0, 0, tdm_smem_a, tdm_smem_b, BLOCK_SIZE_K, NUM_BUFFERS, False)
            # async wait until only a and b in queue
            #gl.amd.gfx1250.tdm.async_wait(2) # wait until ONLY 2 are in the queue. other two loads have passed
            # load a from the previous issue loads call
            # a = tdm_smem_a.index(consumer % NUM_BUFFERS).load(layout=dot_a_layout)
            # # load b from the previous issue loads call
            # b = tdm_smem_b.index(consumer % NUM_BUFFERS).load(layout=dot_b_layout)
