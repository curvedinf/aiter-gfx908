...
def _gemm_a8w8_kernel(
    ...
    shared_a: gl.constexpr = gl.PaddedSharedLayout.with_identity_for(
        [[max(BLOCK_SIZE_K, 256), 16]], [BLOCK_SIZE_M, BLOCK_SIZE_K], [1, 0]
    )
    shared_b: gl.constexpr = gl.PaddedSharedLayout.with_identity_for(
        [[max(BLOCK_SIZE_K, 256), 16]], [BLOCK_SIZE_N, BLOCK_SIZE_K], [1, 0]
    )
    ...
    for i in gl.static_range(NUM_BUFFERS - 1):
        gl.amd.gfx1250.tdm.async_load(
            a_desc, [offs_am, i * BLOCK_SIZE_K], a_buffer.index(i)
        )
        gl.amd.gfx1250.tdm.async_load(
            b_desc, [offs_bn, i * BLOCK_SIZE_K], b_buffer.index(i)
        )
    ...
    for k in range(NUM_BUFFERS - 1, num_k_iters):
        ...
        gl.amd.gfx1250.tdm.async_load(
            a_desc, [offs_am, k * BLOCK_SIZE_K], a_buffer.index(k % NUM_BUFFERS)
        )
        gl.amd.gfx1250.tdm.async_load(
            b_desc, [offs_bn, k * BLOCK_SIZE_K], b_buffer.index(k % NUM_BUFFERS)
        )

        gl.amd.gfx1250.tdm.async_wait(
            (NUM_BUFFERS - 1) * 2
        )  # wait for a and b async load
        a = a_buffer.index((k - (NUM_BUFFERS - 1)) % NUM_BUFFERS).load(
            layout=dot_a_layout
        )
        b = (
            b_buffer.index((k - (NUM_BUFFERS - 1)) % NUM_BUFFERS)
            .permute([1, 0])
            .load(layout=dot_b_layout)
        )  # wmma layout expects (K, N)
        acc = gl.amd.gfx1250.wmma(a, b, acc)

    # epilogue
    for i in gl.static_range(NUM_BUFFERS - 1):
        gl.amd.gfx1250.tdm.async_wait((NUM_BUFFERS - 2 - i) * 2)
        a = a_buffer.index((num_k_iters - (NUM_BUFFERS - 1) + i) % NUM_BUFFERS).load(
            layout=dot_a_layout
        )
        b = (
            b_buffer.index((num_k_iters - (NUM_BUFFERS - 1) + i) % NUM_BUFFERS)
            .permute([1, 0])
            .load(layout=dot_b_layout)
        )
        acc = gl.amd.gfx1250.wmma(a, b, acc)
