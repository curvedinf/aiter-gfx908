...
def _fwd_grouped_kernel_stage1_n16x4_prefetch_k(
    ...
    blocked_v: gl.constexpr = gl.DistributedLinearLayout(
        ...
        lane_bases=[[0, 4], [0, 8], [0, 16], [0, 32], [16, 0], [32, 0]],
        warp_bases=[[0, 64], [0, 128]],
        block_bases=[],
        shape=[64, 256]
    )

    blocked_o: gl.constexpr = gl.BlockedLayout(  # max 64 * 512 in
        size_per_thread=[1, 4],  # 64 * 512
        threads_per_warp=[4, 16],
        warps_per_cta=[1, 4],
        order=[1, 0],
    )
    ...
    shared_q_rope: gl.constexpr = gl.SwizzledSharedLayout(
        vec=8, per_phase=1, max_phase=16, order=[1, 0]
    )
    shared_k_nope: gl.constexpr = gl.SwizzledSharedLayout(
    ...
    shared_o: gl.constexpr = gl.SwizzledSharedLayout(
        vec=4, per_phase=1, max_phase=16, order=[1, 0]
    )

    smem_q0_nope = gl.allocate_shared_memory(
        Q.type.element_ty, [BLOCK_H, kv_lora_rank // 2], layout=shared_q_nope
    )
    smem_q1_nope = gl.allocate_shared_memory(
        Q.type.element_ty, [BLOCK_H, kv_lora_rank // 2], layout=shared_q_nope
    )
    smem_q_rope = gl.allocate_shared_memory(
        Q.type.element_ty, [BLOCK_H, qk_rope_head_dim], layout=shared_q_rope
    )
    ...
    kv_indices += split_kv_start + cur_batch_kv_start_idx
    ...
    kv_pe_loc = gl.amd.cdna3.buffer_load(
        ptr=kv_indices,
        offsets=cur_N_pe
    )
    q = gl.amd.cdna3.buffer_load(
    ...
    kv_loc = gl.convert_layout(kv_pe_loc, gl.SliceLayout(1, blocked_ld_k_nope_kn))
    smem_q0_nope.store(q0)
    smem_q1_nope.store(q1)
    ...
    smem_kv1 = gl.allocate_shared_memory(
        K_Buffer.type.element_ty, [kv_lora_rank // 2, BLOCK_N], layout=shared_k_nope
    )
    smem_kv2 = gl.allocate_shared_memory(
        K_Buffer.type.element_ty, [kv_lora_rank // 2, BLOCK_N], layout=shared_k_nope
    )
    ...
    kv_pe_loc = gl.amd.cdna3.buffer_load(
        ptr = kv_indices,
        offsets = cur_N_pe + BLOCK_N
    )
    # kv_loc = gl.amd.cdna3.buffer_load(
    #     ptr = kv_indices,
    #     offsets = cur_N + BLOCK_N
    # )
    smem_kv1.store(kv1.T)
    ...
    for start_n in range(BLOCK_N, split_kv_end, BLOCK_N):
        cur_k_pe = smem_k_rope.load(layout=dot_k_layout)
        cur_k1 = smem_kv1.load(layout=dot_k_layout)
        ...
        cur_k1 = gl.reshape(cur_k1, (64, 64, 4))
        ...
        cur_k2 = gl.reshape(cur_k2, (64, 64, 4))
        ...
        if start_n < split_kv_end - BLOCK_N:
            kv_pe_loc = gl.amd.cdna3.buffer_load(
                ptr = kv_indices,
                offsets = cur_N_pe + start_n + BLOCK_N,
            )
            kv_loc = gl.amd.cdna3.buffer_load(
                ptr = kv_indices,
                offsets = cur_N + start_n + BLOCK_N,
            )
        ...
        cur_p = tl.broadcast_to(cur_p, (4, 16, 64))
        e_max = n_e_max
    ...
    acc1 = gl.amd.cdna3.mfma(cur_p, cur_k1, acc1)
    acc1  = gl.permute(acc1, (1, 2, 0))
    acc1 = gl.reshape(acc1, (BLOCK_H, BLOCK_C //2))

    e_sum_rev = gl.convert_layout(e_sum_rev, gl.SliceLayout(1, acc1.type.layout))

    acc2 = gl.amd.cdna3.mfma(cur_p, cur_k2, acc2)
    acc2  = gl.permute(acc2, (1, 2, 0))
    acc2 = gl.reshape(acc2, (BLOCK_H, BLOCK_C //2))

    smem_p._keep_alive()

    smem_o1 = gl.allocate_shared_memory(
        Att_Out.type.element_ty, [BLOCK_H, kv_lora_rank//2], layout=shared_o
    )
    smem_o2 = gl.allocate_shared_memory(
        Att_Out.type.element_ty, [BLOCK_H, kv_lora_rank//2], layout=shared_o
    )
    smem_o1.store(acc1 * e_sum_rev[:, None])
    smem_o2.store(acc2 * e_sum_rev[:, None])

    cur_head_o = cur_head_id * VALID_BLOCK_H + gl.arange(
        0, BLOCK_H, layout=gl.SliceLayout(1, blocked_o)
    )
    offs_c_o = gl.arange(
        0, BLOCK_C//2, layout=gl.SliceLayout(0, blocked_o)
    )
    offs_mid_o = (
        cur_batch * stride_mid_ob
        + cur_head_o[:, None] * stride_mid_oh
        + split_kv_id * stride_mid_os
        + offs_c_o[None, :]
    )

    cur_o1 = smem_o1.load(layout=blocked_o)
    cur_o2 = smem_o2.load(layout=blocked_o)
