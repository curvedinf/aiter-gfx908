...
def _fwd_grouped_kernel_stage1_n16x4_prefetch_k_paged_64(
    ...
    # 优化: 提前计算并缓存batch相关指针以减少重复计算
    cur_batch_kv = cur_batch # // (max_qo_len // 2)
    kv_indptr_base = kv_indptr + cur_batch_kv
    cur_batch_kv_start_idx = gl.load(kv_indptr_base)
    cur_batch_kv_end_idx = gl.load(kv_indptr_base + 1)
    ...
    # 优化: 提前计算基础偏移量避免重复计算
    cur_batch_offset = cur_batch * stride_qb
    off_q_pe = (
        cur_batch_offset + cur_head_pe[:, None] * stride_qh + offs_qk_r[None, :]
    )
    offs_q = cur_batch_offset + cur_head[:, None] * stride_qh + offs_c[None, :]
    ...
    # 优化: 预计算常用的mask和索引基址
    kv_indices_base = kv_indices + cur_batch * stride_b_block_table
    ...
    # 优化: 提前计算倒数避免在store时重复除法
    e_sum = 1 / e_sum
    smem_o.store(acc * e_sum[:, None])
    cur_o = smem_o.load(layout=blocked_q_nope_mk)

    # 优化: 预计算批处理偏移避免重复计算
    cur_batch_out_offset = cur_batch * stride_mid_ob + split_kv_id * stride_mid_os
    offs_mid_o = (
        cur_batch_out_offset
...
def _fwd_grouped_kernel_stage1_n16x4_prefetch_k_paged_64_async(
    ...
    cur_k2 = smem_kv2.load(layout=dot_k_layout)
    # if pid == 0:
    #     tl.device_print("cur_k2", cur_k2)
    #     tl.device_print("offs_buf_kv", offs_buf_kv)
    ...
    mask_lse = offs_om < (cur_head_id + 1) * VALID_BLOCK_H
    # if pid == 1:
    #     # tl.device_print("e_max", e_max)
    #     tl.device_print("mask_lse", mask_lse)
...
#     # )
# 
#     blocked_ld_k_async: gl.constexpr = gl.BlockedLayout(  # max 16 * 576 for per wave
#         size_per_thread=[1, 8],  # 64 * 576
#         threads_per_warp=[16, 4],
#         warps_per_cta=[4, 1],
#         order=[1, 0],
...
#     mask_h = mask_h & (cur_head < q_head_num)
# 
#     TILE_C: gl.constexpr = 256 // 8
# 
#     cur_N = gl.arange(
#         0, BLOCK_N, layout=gl.SliceLayout(1, blocked_ld_k_async)
#     )
# 
#     cur_head_pe = gl.arange(
#         0, BLOCK_H, layout=gl.SliceLayout(1, blocked_q_rope_mk)
...
#     # )
#     offs_k_c = gl.arange(
#         0, TILE_C, layout=gl.SliceLayout(0, blocked_ld_k_async)
#     )
#     offs_k_r = gl.arange(
#         kv_lora_rank, kv_lora_rank + BLOCK_R, layout=gl.SliceLayout(0, blocked_ld_k_async)
#     )  # to get the k_pe
# 
#     cur_batch_seq_len = cur_batch_kv_end_idx - cur_batch_kv_start_idx
#     cur_batch_block_nums = gl.cdiv(cur_batch_seq_len, PAGE_BLOCK_SIZE)
...
#     )
# 
#     smem_kv10 = smem_kv1.slice(0,          TILE_C, 1)
#     smem_kv11 = smem_kv1.slice(TILE_C,     TILE_C, 1)
#     smem_kv12 = smem_kv1.slice(TILE_C * 2, TILE_C, 1)
#     smem_kv13 = smem_kv1.slice(TILE_C * 3, TILE_C, 1)
#     smem_kv14 = smem_kv1.slice(TILE_C * 4, TILE_C, 1)
#     smem_kv15 = smem_kv1.slice(TILE_C * 5, TILE_C, 1)
#     smem_kv16 = smem_kv1.slice(TILE_C * 6, TILE_C, 1)
#     smem_kv17 = smem_kv1.slice(TILE_C * 7, TILE_C, 1)
# 
#     smem_kv20 = smem_kv2.slice(0,          TILE_C, 1)
#     smem_kv21 = smem_kv2.slice(TILE_C,     TILE_C, 1)
#     smem_kv22 = smem_kv2.slice(TILE_C * 2, TILE_C, 1)
#     smem_kv23 = smem_kv2.slice(TILE_C * 3, TILE_C, 1)
#     smem_kv24 = smem_kv2.slice(TILE_C * 4, TILE_C, 1)
#     smem_kv25 = smem_kv2.slice(TILE_C * 5, TILE_C, 1)
#     smem_kv26 = smem_kv2.slice(TILE_C * 6, TILE_C, 1)
#     smem_kv27 = smem_kv2.slice(TILE_C * 7, TILE_C, 1)
# 
#     ### =======   load kv1 ====== ###
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv10,
...
#         smem_kv11,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv12,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 2,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv13,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 3,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv14,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 4,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv15,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 5,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv16,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 6,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv17,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 7,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     ### =======   load kv1 end  ====== ###
# 
...
#         smem_kv21,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C + 256,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv22,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 2 + 256,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv23,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 3 + 256,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
# 
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv24,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 4 + 256,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv25,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 5 + 256,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv26,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 6 + 256,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     gl.amd.cdna4.async_copy.buffer_load_to_shared(
#         smem_kv27,
#         K_Buffer,
#         offsets=offs_buf_kv + TILE_C * 7 + 256,
#         mask=mask_k[:, None] & mask_k_c[None, :],
#     )
#     ### =======   load kv2 end  ====== ###
# 
#     offs_buf_k_pe = k_id[:, None] * stride_buf_kh + offs_k_r[None, :]
# 
#     k_pe = gl.amd.cdna4.buffer_load(
#         ptr=K_Buffer,
#         offsets=offs_buf_k_pe,
#         mask=mask_k[:, None] & mask_k_r[None, :]
#     )  # positional embedding part of keys
# 
#     # if pid == 2:
#     #     tl.device_print("k_pe before ", k_pe)
