...
def _paged_attn_decode_v2_w_dot_kernel_reshape_noloop_qk_gluon(
    ...
    MAX_NUM_KV_BLKS: gl.constexpr = triton.cdiv(SEQ_PARTITION_SZ, KV_BLK_SZ)
    ...
    blocked_k: gl.constexpr = gl.BlockedLayout(
        size_per_thread =[1, 1, 1, CONTIGUOUS_KV_ELEMS_16B_LOAD],
    ...
    qk_lhs_layout: gl.constexpr = gl.DotOperandLayout(
        operand_index=0, parent=qk_mfma_layout, k_width=CONTIGUOUS_KV_ELEMS_16B_LOAD
    )
    qk_rhs_layout: gl.constexpr = gl.DotOperandLayout(
        operand_index=1, parent=qk_mfma_layout, k_width=CONTIGUOUS_KV_ELEMS_16B_LOAD
    )
    ...
    num_kv_blks = gl.cdiv(seq_len, KV_BLK_SZ)
    kv_blk_nums = gl.amd.cdna3.buffer_load(ptr=blk_tables_start_ptr, offsets=kv_blk_start + blk_ids, mask=kv_blk_start + blk_ids < num_kv_blks)
    ...
    kt = tl.reshape(kt_temp, [HEAD_SZ_POW2, SEQ_PARTITION_SZ])
    accumulator = gl.zeros((QUERY_GRP_SZ_POW2, SEQ_PARTITION_SZ), dtype=gl.float32, layout=qk_mfma_layout)
    ...
    exp_sum = gl.sum(p, axis=1, keep_dims=True)
    p = p.to(compute_type)
    ...
    gl.amd.cdna3.buffer_store(stored_value=exp_sum, ptr=exp_sums_ptr, offsets=m_l_offs[:, None], mask=m_l_grp_mask[:, None])
    ...
    v = gl.reshape(v, [SEQ_PARTITION_SZ, HEAD_SZ_POW2])
...
def paged_attn_decode_v2(
    ...
    # GQA
        ...
        tmp_output = torch.zeros(
            *shape_info, head_sz, dtype=torch.float32, device=output.device
        )
        if query_grp_sz <= 16:
            query_grp_sz_pow2 = 16
        else:
...
def _paged_attn_decode_v2_w_dot_reduce_kernel(
    ...
    if USE_SINKS:
        M = gl.amd.cdna3.buffer_load(
            ptr=sink_ptr,
            offsets=kv_head_idx * QUERY_GRP_SZ + q_grp_offs,
    ...
    out = tl.sum((logits * gl.convert_layout(p, layout=blocked_layout)), axis=0).to(out_ptr.dtype.element_ty)
...
def _paged_attn_decode_v2_w_dot_reduce_kernel_per_token_quant(
    ...
    # out: [QUERY_GRP_SZ_POW2, HEAD_SZ_POW2]
    out = tl.sum((logits.to(tl.float32) * p), axis=0).to(out_ptr.dtype.element_ty)
