"""
Gluon-based Sage Attention MXFP4 forward kernel for CDNA4 (gfx950).

This is an experimental port of _sage_fwd_no_mask_mxfp4 to the Gluon API,
using explicit layout control and mfma_scaled for the Q·K dot product.

Key differences from the standard Triton kernel:
  - Uses mfma_scaled (instr [32,32,64]) for Q·K with e2m1 format
  - Uses mfma_cdna4 (instr [32,32,16]) for P·V with fp8 format
  - Two separate MMA layouts with layout conversion between QK and PV
  - Explicit shared memory staging for Q, Q_scale, K, K_scale, V
"""
...
# ---------------------------------------------------------------------------
# Inner loop: full (no-mask) blocks
# ---------------------------------------------------------------------------

@gluon.jit
def _gluon_sage_inner_no_mask(
    acc, l_i, m_i,
    q_dot, q_scale_dot,
    k_base, v_base,
    ks_base,
    ...
    """
    Inner loop for full (no-boundary) blocks.
    m_i and l_i are kept in qk_mma_m_layout to avoid per-iteration conversions.
    Simple loop: gl.load for K, buffer_load for V issued early.
    """
    ...
    # Slice layouts for K global loads
    ...
    for block_n in tl.range(block_start, block_end):
        ...
        # Issue V buffer_load first (longest latency, overlaps with everything)
        ...
        # Load K + K_scale via buffer_load → smem
        ...
        if PADDED_HEAD_QK:
            kt_regs = gl.amd.cdna4.buffer_load(ptr=k_base, offsets=kt_offsets, mask=kt_mask)
        else:
            kt_regs = gl.amd.cdna4.buffer_load(ptr=k_base, offsets=kt_offsets)
        ks_regs = gl.amd.cdna4.buffer_load(ptr=ks_base, offsets=ks_offsets)
        k_smem.store(kt_regs)
        ks_smem.store(ks_regs)

        # Load K + K_scale from smem in dot layouts
        ...
        # Online softmax (m_i/l_i in qk_mma_m_layout — no conversion needed)
        ...
        # P conversion + V smem staging + P·V
        v_smem.store(v_regs)
...
# ---------------------------------------------------------------------------
# Inner loop: masked blocks (boundary / causal)
# ---------------------------------------------------------------------------

@gluon.jit
def _gluon_sage_inner_mask(
    acc, l_i, m_i,
    q_dot, q_scale_dot,
    k_base, v_base,
    ks_base,
    ...
    for block_n in range(block_start, block_end):
        ...
        # --- Issue V load early (overlaps with QK+softmax) ---
        v_offsets = (start_n + v_offs_n[:, None]) * stride_vk + v_offs_d[None, :] * stride_vn
        ...
        # --- QK with pre-masking for padding ---
        qk = gl.zeros([BLOCK_M, BLOCK_N], dtype=gl.float32, layout=qk_mma_layout)
        ...
        # --- Online softmax (m_i/l_i already in qk_mma_m_layout) ---
        m_ij = gl.max(qk, axis=1)
        m_new = gl.maximum(m_i, m_ij)
...
def gluon_sage_mxfp4_fwd(
    ...
    Uses two MMA layouts:
      - qk_mma (instr [32,32,64]): for mfma_scaled Q·K with e2m1
      - pv_mma (instr [32,32,16]): for mfma P·V with fp8
    """
    SCALE_GROUP: gl.constexpr = 32
    ...
    # MMA layout for PV: standard mfma with fp8 (K=16)
    ...
    # ===================== Init accumulators =====================
    # m_i and l_i live in qk_mma_m_layout to avoid per-iteration conversions
    # in the softmax; only alpha is converted to pv_mma_m_layout once per iter.
    ...
    if n_full_blocks > 0:
        acc, l_i, m_i = _gluon_sage_inner_no_mask(
            acc, l_i, m_i,
            q_dot, q_scale_dot,
    ...
    if masked_blocks > 0:
        acc, l_i, m_i = _gluon_sage_inner_mask(
            acc, l_i, m_i,
            q_dot, q_scale_dot,
            k_base, v_base, ks_base, bias_base,
            stride_kn, stride_kk,
