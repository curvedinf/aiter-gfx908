"""
Wrapper and benchmark for the Gluon-based Sage Attention MXFP4 kernel.
...
def gluon_sage_mxfp4_func(
    ...
    assert layout in ["bhsd", "bshd"], "Only bhsd and bshd supported"
...
def benchmark_gluon_sage_mxfp4(args):
    ...
    # Build Gluon config with optional overrides
    ...
    if args.compare_to_ref:
        # Compare to original Triton sage_mxfp4 (same quantization path)
        from aiter.ops.triton.attention.fav3_sage_attention_mxfp4_wrapper import (
        ...
        max_diff_t = (out_gluon.float() - out_triton.float()).abs().max().item()
        print(f"vs Triton sage_mxfp4: cosine={cos_sim_t.item():.6f}  max_diff={max_diff_t:.6f}")

        # Also compare to fp32 reference
        ref_out = attention_ref(q, k, v, causal=args.causal)
        cos_sim_ref = torch.nn.functional.cosine_similarity(
            out_gluon.reshape(-1).float(),
            ref_out.reshape(-1).float(),
            dim=0,
        )
        print(f"vs fp32 reference:    cosine={cos_sim_ref.item():.6f}")
        print()
    ...
    # Benchmark original Triton for comparison
    from aiter.ops.triton.attention.fav3_sage_attention_mxfp4_wrapper import (
        fav3_sage_mxfp4_func,
    )

    # Re-quantize for Triton with its own BLOCK_M
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark Gluon Sage MXFP4 Attention")
    ...
    parser.add_argument("--block-m", type=int, default=None, help="Override BLOCK_M for Gluon")
    parser.add_argument("--block-n", type=int, default=None, help="Override BLOCK_N for Gluon")
    parser.add_argument("--warps", type=int, default=None, help="Override num_warps for Gluon")
    parser.add_argument("--waves", type=int, default=None, help="Override waves_per_eu for Gluon")
    args = parser.parse_args()
    benchmark_gluon_sage_mxfp4(args)
