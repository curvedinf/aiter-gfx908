...
class AttentionConfig:
    ...
    def __init__(self, HEAD_SIZE, BLOCK_SIZE, BLOCK_M, BLOCK_Q,
                 NUM_QUERY_HEADS, NUM_KV_HEADS, SLIDING_WINDOW,
                 NUM_WARPS, WARP_SIZE, SCALE,
                 USE_LOAD_BUFFER_OP, USE_STORE_BUFFER_OP):
        ...
        # shared memory layouts for K and V
        self.shared_k_layout = gl.constexpr(
            gl.PaddedSharedLayout.with_identity_for([[HEAD_SIZE, 8]], [BLOCK_SIZE, HEAD_SIZE], [1, 0]))
        self.shared_v_layout = gl.constexpr(
            gl.PaddedSharedLayout.with_identity_for([[HEAD_SIZE, 16]], [BLOCK_SIZE, HEAD_SIZE], [1, 0]))
...
class AttentionProgram:
    ...
    def initialize(cfg, q,
        ...
        k_shared = gl.allocate_shared_memory(
            key_cache_ptr.type.element_ty, [2, cfg.BLOCK_SIZE, cfg.HEAD_SIZE], layout=cfg.shared_k_layout)
    ...
    def tdm_shared_load_k(self, wait_count, buffer_id):
        ...
        return self.k_shared.index(buffer_id).permute([1, 0]).load(layout=self.cfg.k_layout)

    @gluon.jit
    def tdm_shared_load_v(self, wait_count, buffer_id):
        gl.amd.gfx1250.tdm.async_wait(wait_count)
        return self.v_shared.index(buffer_id).load(layout=self.cfg.v_layout)
...
def kernel_unified_attention_2d(
    ...
    cfg = AttentionConfig(HEAD_SIZE, BLOCK_SIZE, BLOCK_M, BLOCK_Q,
                          NUM_QUERY_HEADS, NUM_KV_HEADS, SLIDING_WINDOW,
                          NUM_WARPS, WARP_SIZE, SCALE,
                          USE_LOAD_BUFFER_OP, USE_STORE_BUFFER_OP)
    ...
    for j in range(pgm.tile_start, pgm.tile_end - 1):
        ...
        pgm.tdm_load_global_to_shared_k(next_physical_block_idx, buffer_id=1-buffer_id)
        pgm.tdm_load_global_to_shared_v(next_physical_block_idx, buffer_id=1-buffer_id)
...
def unified_attention(q,
    ...
    USE_STORE_BUFFER_OP = USE_LOAD_BUFFER_OP
