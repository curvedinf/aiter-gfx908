...
def _allocate_L_M_acc(
    ...
    acc = gl.zeros([BLOCK_M, HEAD_SIZE], dtype=tl.float32, layout=PV_WMMA_LAYOUT)
    # acc : shape = (BLOCK_M, HEAD_SIZE), layout = PV_WMMA_LAYOUT
    acc = gl.zeros([BLOCK_M, HEAD_SIZE], dtype=tl.float32, layout=PV_WMMA_LAYOUT)
...
#         block_shape=(NUM_BLOCKS_GATHER_PER_TILE, BLOCK_SIZE * HEAD_SIZE),
...
#     HEAD_SIZE: gl.constexpr,  # int, must be power of 2
...
#     offs_q_d = gl.arange(0, HEAD_SIZE, layout=gl.SliceLayout(0, Q_LOAD_LAYOUT))
#     offs_q_m = gl.arange(0, BLOCK_M, layout=gl.SliceLayout(1, Q_LOAD_LAYOUT))
#     offs_q_d = gl.arange(0, HEAD_SIZE, layout=gl.SliceLayout(0, Q_LOAD_LAYOUT))
...
#     # Q_load : shape = (BLOCK_M, HEAD_SIZE), layout = Q_LOAD_LAYOUT
...
#     )
#     smem_Q.store(Q_load)
#     # Q : shape = (BLOCK_M, HEAD_SIZE), layout = Q_DOT_LAYOUT
...
#     # store segm_output
#     # acc : shape = (BLOCK_M, HEAD_SIZE), layout = Q_LOAD_LAYOUT
#     acc = gl.convert_layout(acc, layout=Q_LOAD_LAYOUT)
#     segm_output_offset = (
#         query_offset_0[:, None]
#         * (num_query_heads * NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
#         + query_offset_1[:, None] * (NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
#         + segm_idx * HEAD_SIZE
#         + offs_q_d[None, :]
#     )
...
def gluon_kernel_unified_attention_3d_async(
    ...
    smem_Q = gl.allocate_shared_memory(
        query_ptr.type.element_ty, [BLOCK_M, HEAD_SIZE], layout=Q_SHARED_LAYOUT
    smem_Q = gl.allocate_shared_memory(
        query_ptr.type.element_ty, [BLOCK_M, HEAD_SIZE], layout=Q_SHARED_LAYOUT
    )
    smem_K = None
    ...
    if SHUFFLED_KV_CACHE:
        smem_K = gl.allocate_shared_memory(
            ...
            [num_stages, TILE_SIZE // 16, HEAD_SIZE * 16],
            layout=K_SHARED_LAYOUT,
        )
        smem_V = gl.allocate_shared_memory(
            value_cache_ptr.type.element_ty,
            [num_stages, HEAD_SIZE // 16, TILE_SIZE * 16],
            layout=V_SHARED_LAYOUT,
        )
    else:
        smem_K = gl.allocate_shared_memory(
            key_cache_ptr.type.element_ty,
            [num_stages, HEAD_SIZE, TILE_SIZE],
            layout=K_SHARED_LAYOUT,
        )
        smem_V = gl.allocate_shared_memory(
            value_cache_ptr.type.element_ty,
            [num_stages, TILE_SIZE, HEAD_SIZE],
            layout=V_SHARED_LAYOUT,
        )
    smem_K = None
    smem_V = None
    ...
    if SHUFFLED_KV_CACHE:
        ...
        offs_v_d = gl.arange(
            0, HEAD_SIZE // 16, layout=gl.SliceLayout(1, V_LOAD_LAYOUT)
        )
    else:
        offs_k_t = gl.arange(0, TILE_SIZE, layout=gl.SliceLayout(0, K_LOAD_LAYOUT))
        offs_k_d = gl.arange(0, HEAD_SIZE, layout=gl.SliceLayout(1, K_LOAD_LAYOUT))
        offs_v_t = gl.arange(0, TILE_SIZE, layout=gl.SliceLayout(1, V_LOAD_LAYOUT))
        offs_v_d = gl.arange(0, HEAD_SIZE, layout=gl.SliceLayout(0, V_LOAD_LAYOUT))
    ...
    for _ in range(num_stages - 1):
        ...
        k_from_hbm = _async_load_to_lds(
            ...
            use_buffer_load=USE_LOAD_BUFFER_OP,
            use_buffer_load=USE_LOAD_BUFFER_OP,
        )
    ...
    for _ in range(
        ...
        # )
        # if SHUFFLED_KV_CACHE:
        #     K = _unshuffle_kv_cache(K, TILE_SIZE, HEAD_SIZE)
        ...
        if SHUFFLED_KV_CACHE:
            V = _unshuffle_kv_cache(V, HEAD_SIZE, TILE_SIZE)
    ...
    acc = gl.convert_layout(acc, layout=Q_LOAD_LAYOUT)
    segm_output_offset = (
        query_offset_0[:, None] * (NUM_QUERY_HEADS * NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
        + query_offset_1[:, None] * (NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
        + segm_idx * HEAD_SIZE
        query_offset_0[:, None] * (NUM_QUERY_HEADS * NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
        + query_offset_1[:, None] * (NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
...
def gluon_kernel_unified_attention_3d(
    ...
    if SHUFFLED_KV_CACHE:
        ...
        smem_K = gl.allocate_shared_memory(
            key_cache_ptr.type.element_ty,
            [TILE_SIZE // 16, HEAD_SIZE * 16],
            layout=K_SHARED_LAYOUT,
        )
        smem_V = gl.allocate_shared_memory(
            value_cache_ptr.type.element_ty,
            [HEAD_SIZE // 16, TILE_SIZE * 16],
            layout=V_SHARED_LAYOUT,
        )
    else:
        smem_K = gl.allocate_shared_memory(
    ...
    offs_q_m_qk = gl.arange(0, BLOCK_M, layout=gl.SliceLayout(1, QK_WMMA_LAYOUT))
    offs_q_d_qk = gl.arange(0, HEAD_SIZE, layout=gl.SliceLayout(0, QK_WMMA_LAYOUT))
    query_pos_qk = q_block_local_idx * BLOCK_Q + offs_q_m_qk // num_queries_per_kv
    ...
    offs_q_d_pv = gl.arange(0, HEAD_SIZE, layout=gl.SliceLayout(0, PV_WMMA_LAYOUT))
    ...
    segm_output_offset = (
        ...
        * (NUM_QUERY_HEADS * NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
        + query_offset_1_pv[:, None] * (NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
        + segm_idx * HEAD_SIZE
        + offs_q_d_pv[None, :]
        query_offset_0_pv[:, None]
        * (NUM_QUERY_HEADS * NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
...
def gluon_reduce_segments(
    ...
    segm_output_offset = (
        ...
        * (num_query_heads * NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
        + query_head_idx * (NUM_SEGMENTS_PER_SEQ * HEAD_SIZE)
        + tl.arange(0, NUM_SEGMENTS_PER_SEQ)[:, None] * HEAD_SIZE
