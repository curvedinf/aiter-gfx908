...
def topk_forward(x, k, apply_softmax=True, dim=1, return_bitmatrix=True, y_indx=None):
    ...
    bitmatrix = Bitmatrix(bitmatrix, shape=bitmatrix_shape, scratchpad=scratchpad)
...
def topk(x, k, apply_softmax=True, dim=1, return_bitmatrix=True, y_indx=None):
    ret = topk_forward(x, k, apply_softmax, dim, return_bitmatrix, y_indx)
    return ret
