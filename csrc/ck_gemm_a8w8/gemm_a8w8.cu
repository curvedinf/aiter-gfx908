...
// Generated by gen_instances.py: declares which (AB,D,E) dtype combos have
// instances in this build. Under dtype pruning (AITER_CK_DTYPES / gfx908
// default), pruned combos compile to a loud TORCH_CHECK instead of
// referencing never-emitted kernel symbols (which would fail at import --
// .so links permit undefined symbols).
#include "gemm_a8w8_built_combos.h"

#ifdef AITER_PRUNE_DTYPES
#define AITER_PRUNED_COMBO(AB, D, E) \
  TORCH_CHECK(false, "a8w8 dtype combo <" #AB "," #D "," #E "> is pruned from this build (see AITER_CK_DTYPES / gemm_a8w8_built_combos.h)")
#else
#define AITER_PRUNED_COMBO(AB, D, E)
#endif
...
#else
      AITER_PRUNED_COMBO(F8, F32, B16);
#endif
    }
    else if (Y.dtype() == at::ScalarType::Half)
    {
#ifdef AITER_BUILT_F8_F16_F16
      rowwise_dispatch<F8, F16, F16>(M, N, K)(XQ, WQ, x_scale, w_scale, Y, bias, KBatch);
#else
      AITER_PRUNED_COMBO(F8, F16, F16);
#endif
    }
    else if (Y.dtype() == at::ScalarType::BFloat16)
    {
#ifdef AITER_BUILT_F8_B16_B16
      rowwise_dispatch<F8, B16, B16>(M, N, K)(XQ, WQ, x_scale, w_scale, Y, bias, KBatch);
#else
      AITER_PRUNED_COMBO(F8, B16, B16);
#endif
    }
    else
    {
