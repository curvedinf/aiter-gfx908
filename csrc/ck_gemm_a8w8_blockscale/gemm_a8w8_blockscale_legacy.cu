...
static BlockwiseKernel blockscale_dispatch(int M, int N, int K)
{
    ...
    return a8w8_blockscale_legacy_1x128x128_256x16x128x256_16x16_16x16_1x2_16x16x1_16x16x1_1x16x1x16_8_1x2_intrawave_v1<
        DDataType,
        EDataType>;
}
                                          ...
                                          torch::Tensor& Y)
{
    ...
    {
        blockscale_dispatch<LEGACY_FP32, LEGACY_FP16>(M, N, K)(XQ, WQ, x_scale, w_scale, Y);
    }
    else if(x_scale.dtype() == at::ScalarType::Float && Y.dtype() == at::ScalarType::BFloat16)
    {
        blockscale_dispatch<LEGACY_FP32, LEGACY_BF16>(M, N, K)(XQ, WQ, x_scale, w_scale, Y);
    }
    else
    {
