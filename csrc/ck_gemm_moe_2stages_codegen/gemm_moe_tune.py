...
class FmoeTuner(TunerCommon):
    ...
    def gen_2stages_flydsl_task(self, key, blockMs, flydsl_csv=None):
        """Generate FlyDSL 2-stage tuning tasks."""
        info = key
        tasks_flydsl = []
        (
            cu_num,
    ...
    def tune(
        ...
        for line in untunedf[keys].values:
            ...
            if (
                tasks is None
                and tasks_ck is None
                and task_1stage is None
                and tasks_flydsl is None
            ):
                print("no moe solution can tune for ", line)
                continue
            print(
                f"stage1 asm tasks is {len(tasks)}, tasks_ck is {len(tasks_ck)}, tasks_flydsl is {len(tasks_flydsl)}, task_1stage is {len(task_1stage)}"
            )
        all_tasks = tasks + tasks_ck + tasks_flydsl + task_1stage
    ...
    def post_process(self, results, args, topk=-1, fast_mode=False):
        ...
        for key, rets in grouped_results:
            ...
            stage1_profileDF = profileDF[
                profileDF["stage"].isin(["stage1", "flydsl_stage1"])
            ].drop(columns=["stage"], axis=1)
            ...
            stage2_profileDF = profileDF[
                profileDF["stage"].isin(["stage2", "flydsl_stage2"])
            ].drop(columns=["stage", "ksplit"], axis=1, errors="ignore")
