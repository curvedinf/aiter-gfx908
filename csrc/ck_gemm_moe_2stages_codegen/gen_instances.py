...
A8W8_blockscale_gemm2_heuristic_dispatch = """
    ...
    {{
        if (inter_dim % 128 != 0 && inter_dim % 64 == 0)
        {{
            // KPerBlock=64: inter_dim is 64-aligned but not 128-aligned (e.g. tp=4 inter_dim=320)
            // K=inter_dim, uses same per_1x128 scale via k_block_id*64/128 integer division
            if (block_m == 16)
            {{
                if (inter_dim % 256 == 0)
            ...
            {{
                TORCH_CHECK(
                    false,
                    "Unsupported block_m value for KPerBlock=64 blockscale dispatch: ",
                    block_m);
            }}
        }}
        else
