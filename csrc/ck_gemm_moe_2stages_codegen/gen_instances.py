...
A8W8_blockscale_gemm1_heuristic_dispatch = """
    ...
    {{
    if (block_m == 16) {{
        return ck_moe_stage1_gemm<{A0DataType}, {B0DataType}, {AccDataType}, {EDataType}, {CDEElementOp},
                                  V1, 256, 16, 128, 256/sizeof({A0DataType}), 1, 4, {Nswizzle},
                                  ({Quant} == static_cast(QuantType::per_Tensor)),
                                  {MulRoutedWeight}, {ActOP}>;
    }}
    else if (block_m == 64)
        {{
...
A8W8_blockscale_gemm2_heuristic_dispatch = """
    ...
    {{
    if (block_m == 16) {{
        return ck_moe_stage2_gemm<{A0DataType}, {B0DataType}, {AccDataType}, {EDataType}, {CDEElementOp},
                                  V1, 128, 16, 128, 128/sizeof({A0DataType}), 1, 4, {Nswizzle},
                                  ({Quant} == static_cast(QuantType::per_Tensor)),
                                  {MulRoutedWeight}, {ActOP}>;
    }}
    else if (block_m == 64)
