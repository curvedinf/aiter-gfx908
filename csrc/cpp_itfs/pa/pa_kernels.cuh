    ...
    const float* __restrict__ fp8_out_scale_ptr)
{
    ...
    if(warpid == 0)
    {
        const float* max_logits_ptr = max_logits +
                                      (seq_idx + mtp) * num_heads * max_num_partitions +
...
#pragma unroll
        ...
        const float* exp_sums_ptr = exp_sums + (seq_idx + mtp) * num_heads * max_num_partitions +
                                    head_idx * max_num_partitions;
...
#pragma unroll
    ...
    const scalar_t* tmp_out_ptr = tmp_out +
                                  (seq_idx + mtp) * num_heads * max_num_partitions * HEAD_SIZE +
