...
// scale_out logical shape: (M, hidden_dim / group_size). When transpose_scale
// is true, write transposed physical storage [group, M] for preshuffle GEMM
// consumers while keeping the exposed tensor shape unchanged.
template <typename P, typename A, typename T, typename OutT, int PACK_SIZE>
...
#pragma unroll
    ...
    {
        int scale_idx = transpose_scale ? group_id * token_num + tidx
                                        : tidx * num_groups + group_id;
        scale_out[scale_idx] = scale;
    }
}
...
#pragma unroll
    ...
    ar_fusion_epilogue_per_group<P, A, T, OutT, pack_size>(
        acc, weight_p, hidden_dim, eps, idx, tidx, token_num,
        padded_block_size, group_size, output, scale_out, transpose_scale,
        active, bf16_output);
}
...
void allreduce_fusion_kernel_1stage_per_group_launcher(
    ...
    float eps, hipStream_t stream, bool transpose_scale = false, T* bf16_output = nullptr)
{
...
#pragma unroll
        for(int v = 0; v < pack_size; ++v)
            acc[v] = upcast_s(vec[v]);
        ar_fusion_epilogue_per_group<P, A, T, OutT, pack_size>(
            acc, weight_p, hidden_dim, eps, idx, tidx, size / hidden_dim,
            block_size, group_size, output, scale_out, transpose_scale,
    ...
    float eps, hipStream_t stream, bool transpose_scale = false, T* bf16_output = nullptr)
{
    ...
    allreduce_fusion_kernel_2stage_per_group<T, OutT, NGPUS>
        <<<numBlocks, threadsPerBlock, smem_size, stream>>>(
            ...
            size, hidden_dim, group_size, eps, transpose_scale, bf16_output);
}
...
#define DISPATCH_AR_FUSION_PG_KERNEL(NGPUS)                                               \
    ...
    {                                                                                      \
        allreduce_fusion_kernel_1stage_per_group_launcher<T, QT, NGPUS>(                   \
            ...
            residual_inp, residual_out, output, weight, scale_out,                          \
            size, n, group_size, eps, stream, transpose_scale, bf16_output);                 \
        return;                                                                             \
    }                                                                                      \
    else if(n_constrain && (size * sizeof(T) <= 512 * 1024))                               \
    {                                                                                      \
