                                ...
                                const std::vector<torch::Tensor>& offsets)
    {
        ...
        for(int i = 0; i < num_buffers; i++)
        {
            ...
            for(int j = 0; j < world_size_; j++)
            {
                if(j != rank_)
                {
                    cudaIpcMemHandle_t* ipc_handle_ptr =
                        (cudaIpcMemHandle_t*)handles[j].data_ptr() + i;
...
#define DISPATCH_CALL(pack_size, block_size, quant_scale)                                     \
    ...
    {
        blocks =
            std::min(kMaxBlocks, (size + (threads / world_size_) - 1) / (threads / world_size_));
    }
    else if(call_2stage)
    {
        blocks =
            std::min(kMaxBlocks,
                     (size / world_size_ + (threads / world_size_) - 1) / (threads / world_size_));
    }
