...
// Fused AR + residual + RMSNorm + per-group INT8 quantization producing the
// activation format vLLM's W8A8 path consumes on gfx908: out int8 [M,K] and
// scale_out float16 [M, K/group_size] (per-row per-128-block absmax/127, see
// _quantize_activation_per_block in vllm .../mixed_precision/triton_w8a16.py).
// Unlike the fp8 variant above this path uses no fp8 conversion builtins, so
// it is safe on gfx908 where those are compile-only stubs.
void fused_allreduce_rmsnorm_quant_int8_per_group(fptr_t _fa,
                                                  const aiter_tensor_t& inp,
                                                  const aiter_tensor_t& res_inp,
                                                  const aiter_tensor_t& res_out,
                                                  const aiter_tensor_t& out,
                                                  ...
                                                  int64_t bf16_out_ptr)
{
    ...
    {
        throw std::runtime_error(
            "fused_allreduce_rmsnorm_quant_int8_per_group does not support gemma_norm");
    }
    HipDeviceGuard device_guard(inp.device_id);
    hipStream_t stream = aiter::getCurrentHIPStream();
    auto dtype     = inp.dtype();
    ...
    // contract as the fp8 per-group entry.
    void* bf16_out = reinterpret_cast<void*>(bf16_out_ptr);
...
#if(__CUDA_ARCH__ >= 800 || !defined(__CUDA_ARCH__))
    case AITER_DTYPE_bf16: {
        fa->dispatchFusedAllReduceRMSNormQuantPerGroup<opus::bf16_t, opus::i8_t, opus::fp16_t>(
            stream,
            reinterpret_cast<opus::bf16_t*>(inp_ptr),
            reinterpret_cast<opus::bf16_t*>(res_inp.data_ptr()),
            reinterpret_cast<opus::bf16_t*>(res_out.data_ptr()),
            reinterpret_cast<opus::i8_t*>(out.data_ptr()),
            reinterpret_cast<opus::fp16_t*>(scale_out.data_ptr()),
            reinterpret_cast<opus::bf16_t*>(w.data_ptr()),
            (float)eps, m, n, (int)group_size, use_1stage,
...
#endif
    case AITER_DTYPE_fp16: {
        fa->dispatchFusedAllReduceRMSNormQuantPerGroup<opus::fp16_t, opus::i8_t, opus::fp16_t>(
            stream,
            reinterpret_cast<opus::fp16_t*>(inp_ptr),
            reinterpret_cast<opus::fp16_t*>(res_inp.data_ptr()),
            reinterpret_cast<opus::fp16_t*>(res_out.data_ptr()),
    ...
    default:
        throw std::runtime_error(
            "fused_allreduce_rmsnorm_quant_int8_per_group only supports float16 and bfloat16");
    }
}
