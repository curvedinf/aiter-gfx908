...
#pragma unroll
    for (int ii = 0; ii < LANE_HIDDEN_SIZE; ++ii)
    {
        ...
        // residual_in_local[ii] = in_local[ii] * scale_rms * gamma_local_fp32[ii];
    }
...
__global__ void no_fused_rms_norm_kernel(NoFusedRMSNormParameter params)
{
    ...
    const int64_t warp_base_row = blockIdx.x;
...
#pragma unroll
    ...
    float gamma_local_fp32[LANE_HIDDEN_SIZE];
    AccVecType* gamma_fp32 = reinterpret_cast<AccVecType*>(&gamma_local_fp32);
    // Issue gamma with add.
#pragma unroll
    for(int32_t ii = 0; ii < VEC_HIDDEN_SIZE_LOC; ++ii)
    {
...
#pragma unroll
    for (int32_t ii = 0; ii < VEC_HIDDEN_SIZE_LOC; ++ii)
    {
        gamma_fp32[ii] = ck_tile::vec_convert<ACC_DTYPE, DTYPE, WIDTH>(gamma_in_ptr[ii]);
    }

    int64_t stride = gridDim.x;
...
#pragma unroll  // Unroll loop for better instruction pipelining
        for (int32_t ii = 0; ii < VEC_HIDDEN_SIZE_LOC; ++ii)
        {
            // row_in_ptr[ii + ld_stage * LANE_HIDDEN_SIZE] = ck_tile::vec_convert<ACC_DTYPE, DTYPE, WIDTH>(row_in_b16_ptr[ii + ld_stage * LANE_HIDDEN_SIZE]);
#pragma unroll
            for (int32_t j = 0; j < WIDTH; ++j)
            {
                in_local_b16[ii * WIDTH + j + st_stage * LANE_HIDDEN_SIZE] = __builtin_nontemporal_load(thread_in_ptr + ii * blockDim * WIDTH + j + stride * params.stride * (r + 1));
            }
        }
...
#pragma unroll
            ...
            {
                auto idx = ii * WIDTH + j;
                in_local[idx] = in_local[idx] * scale_rms * gamma_local_fp32[idx];
            }
        }
...
#pragma unroll  // Unroll loop for better instruction pipelining
        for (int32_t ii = 0; ii < VEC_HIDDEN_SIZE_LOC; ++ii)
        {
            row_in_b16_ptr[ii + ld_stage * VEC_HIDDEN_SIZE_LOC] = ck_tile::vec_convert<QUANT_DTYPE, ACC_DTYPE, WIDTH>(row_in_ptr[ii]);
#pragma unroll
            for (int32_t j = 0; j < WIDTH; ++j)
            {
                // __builtin_nontemporal_store(in_local_b16[ii * WIDTH + j], thread_out_ptr);
                __builtin_nontemporal_store(in_local_b16[ii * WIDTH + j + ld_stage * LANE_HIDDEN_SIZE], thread_out_ptr + ii * blockDim * WIDTH + j + r * HIDDEN_SIZE * stride);
            }
        }
...
#pragma unroll  // Unroll loop for better instruction pipelining
        for (int32_t ii = 0; ii < VEC_HIDDEN_SIZE_LOC; ++ii)
        {
            row_in_ptr[ii] = ck_tile::vec_convert<ACC_DTYPE, DTYPE, WIDTH>(row_in_b16_ptr[ii + st_stage * VEC_HIDDEN_SIZE_LOC]);
        }
    }
...
#pragma unroll
        ...
        {
            // __builtin_nontemporal_store(in_local_b16[ii * WIDTH + j], thread_out_ptr);
	    // __builtin_nontemporal_store(in_local_b16[ii * WIDTH + j + r * LANE_HIDDEN_SIZE], thread_out_ptr + ii * blockDim * WIDTH + j + r * HIDDEN_SIZE);
            __builtin_nontemporal_store(in_local_b16[ii * WIDTH + j + ld_stage * LANE_HIDDEN_SIZE], thread_out_ptr + int64_t(ii * blockDim * WIDTH + j + stride * 3 * HIDDEN_SIZE));
        }
    }
}
              ...
              int use_model_sensitive_rmsnorm = 0)
{
    ...
    if (hidden_size == 8192)
    {
        ...
        dim3 block(std::min(hidden_size, max_block_size));
        no_fused_rms_norm_kernel<ck_tile::bf16_t, 8192, 8, 4, 256, float, ck_tile::bf16_t><<<grid, block, 0, stream>>>(params);
    }
