...
// Temporary: wrapper for separate kernel approach (for benchmarking)
void topk_softmax_separate(torch::Tensor& topk_weights,
                           ...
                           const std::string& shared_expert_scoring_func)
{
    ...
    // First: Process routing experts with softmax + topk (WITHOUT shared expert fusion)
    VLLM_DISPATCH_FLOATING_TYPES(gating_output.scalar_type(), "topk_softmax", [&] {
        ...
        vllm::moe::topkGatingSoftmaxKernelLauncher(
            ...
            num_tokens,
            num_routing_experts,
            0,  // Pass 0 to disable fused shared expert processing
            "",
            topk,
            topk_weights_stride,
            topk_id_stride,
    ...
    // Second: Apply separate sigmoid kernel for shared experts
    if(num_shared_experts > 0 && !shared_expert_scoring_func.empty())
    {
        if(shared_expert_scoring_func == "sigmoid")
        {
            static constexpr int TPB = 256;
            VLLM_DISPATCH_FLOATING_TYPES(gating_output.scalar_type(), "apply_sigmoid_shared", [&] {
                using input_dtype = typename t2ck<scalar_t>::type;
                vllm::moe::applySharedExpertSigmoid<input_dtype, TPB><<<num_tokens, TPB, 0, stream>>>(
                    reinterpret_cast<input_dtype*>(gating_output.data_ptr()),
                    topk_weights.data_ptr<float>() + topk,
                    num_tokens,
                    num_shared_experts,
                    gating_token_stride,
                    topk_weights_stride,
