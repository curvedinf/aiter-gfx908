        ...
        std::optional<at::Generator> gen_)
{
    ...
    if(!deterministic)
    {
        if(is_atomic_fp32)
        {
            dq_accum = torch::zeros({1, batch_size, seqlen_q, num_heads, head_size_v},
    ...
    else
    {
        ...
        dq_accum = torch::zeros({nsplits, batch_size, seqlen_q, num_heads, head_size_v},
                                opts.dtype(at::kFloat));
    }
