...
std::string get_heuristic_kernel(int m_num, int N, int blockk_size, CFG* cfgs, std::string arch_id, int pf, int coexec, int tg, int atomic)
{
    hipDevice_t dev;
    ...
    for(const auto& el : *cfgs)
    {
        ...
        if(atomic == cfg.atomic) // ignore pf for stage2 kernel
        {
            if(blockk_size == 32)
            {
                if(tg == cfg.tg)
...
)
{
    ...
    if(kernelName.empty())
    {
        kernelName = get_heuristic_kernel(sub_X_cnt, inter_dim, block_m, config_map, arch_id, pf, co_exec, -1, -1);
        
        print("[KERNEL] block_m = %d, pf = %d, co_exec = %d\n", block_m, pf, co_exec);
...
)
{
    ...
    if(kernelName.empty())
    {
        kernelName = get_heuristic_kernel(sub_X_cnt, out.size(-1), block_m, config_map, arch_id, -1, -1, tg, do_atomic);

        print("[KERNEL] block_m = %d, tg = %d, do_atomic = %d\n", block_m, tg, do_atomic);
        print("[KERNEL] heuristic_kernel = %s\n", kernelName.c_str());
    }
