                                 ...
                                 CFG* cfgs)
{
    ...
    int real_qtile = qTile;
    if (ps && wave_per_tg == 8 && real_qtile == 40)
        real_qtile = 48;
        ...
        for(int gqa_ : gqa_flags)
        {
            ...
            for(const auto& el : *cfgs)
            {
                ...
                if(cfg.qType == q_type && cfg.kvType == kv_type && cfg.Gqa == gqa_ &&
                   ...
                   cfg.blkSz == block_size && cfg.ps == ps && cfg.qTile == real_qtile && cfg.quant_type == quant_type && cfg.wave_per_tg == wave_per_tg)
