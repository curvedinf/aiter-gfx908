...
class FlyDSLBatchTuner:
    ...
    def generate_test_data(
        ...
        # Weight quantization
        w1_qt, w1_scale = FlyDSLBatchTuner.weight_quant(w1, q_type, quant_dtype=q_dtype_w)
        w2_qt, w2_scale = FlyDSLBatchTuner.weight_quant(w2, q_type, quant_dtype=q_dtype_w)
    ...
    def create_tuned_config(self, untuned_row, tile_row):
        ...
        config['doweight_stage1'] = self.convert_value('doweight_stage1', untuned_row['doweight_stage1'])
    ...
    def run_single_test(self, config, test_data, tile_idx):
        ...
        dtype = eval(config['dtype']) if isinstance(config['dtype'], str) else config['dtype']
        q_dtype_a = eval(config['q_dtype_a']) if isinstance(config['q_dtype_a'], str) else config['q_dtype_a']
        q_dtype_w = eval(config['q_dtype_w']) if isinstance(config['q_dtype_w'], str) else config['q_dtype_w']
        ...
        try:
                ...
                if tile_idx == 0:
                    ...
                    print(f"     kernelName1 = {config['kernelName1']} ← contains 'flydsl', automatically uses FlyDSL")
                ...
                a2_output = torch.zeros((token, topk, inter_dim), dtype=dtype, device='cuda')
                ...
                _, us1_time = run_perftest(
                    ...
                    splitk=0,
                    dtype=dtype,
                    num_iters=101,  # Restore default value
                    num_warmup=2,   # Restore default value   
                )
            ...
            # 🔧 Clean up temporary file and restore environment variable
            if temp_csv_file and os.path.exists(temp_csv_file):
                os.remove(temp_csv_file)
            
            # Restore original environment variable
            if old_aiter_config_fmoe is not None:
                os.environ['AITER_CONFIG_FMOE'] = old_aiter_config_fmoe
            elif 'AITER_CONFIG_FMOE' in os.environ:
                del os.environ['AITER_CONFIG_FMOE']
    ...
    def tune_single_input(self, untuned_idx, untuned_row):
        ...
        print(f"{'='*80}")
        print(f"  token={untuned_row['token']}, model_dim={untuned_row['model_dim']}, "
        ...
        dtype = eval(dtype_str) if isinstance(dtype_str, str) and 'torch.' in dtype_str else dtypes.bf16
        
        q_dtype_a_str = untuned_row['q_dtype_a']
        q_dtype_a = eval(q_dtype_a_str) if isinstance(q_dtype_a_str, str) and 'torch.' in q_dtype_a_str else dtypes.fp8
        
        q_dtype_w_str = untuned_row['q_dtype_w']
        q_dtype_w = eval(q_dtype_w_str) if isinstance(q_dtype_w_str, str) and 'torch.' in q_dtype_w_str else dtypes.fp8
        
        q_type_str = untuned_row['q_type']
        q_type = eval(q_type_str) if isinstance(q_type_str, str) and 'QuantType.' in q_type_str else QuantType.per_Token
        
        use_g1u1 = bool(untuned_row['use_g1u1'])
        
        # Use first tile configuration's block_m to generate test data (shared by all configurations)
        first_tile = self.candidate_tiles.iloc[0]
        block_m = int(first_tile['tile_m'])
        ...
        # Iterate all tile configurations - batch test
        for tile_idx, tile_row in self.candidate_tiles.iterrows():
            config = self.create_tuned_config(untuned_row, tile_row)
            
            print(f"🔧 Testing tile #{tile_idx + 1}/{len(self.candidate_tiles)}: "
                  f"{tile_row['tile_m']}x{tile_row['tile_n']}x{tile_row['tile_k']}", end=' ')
            
            # Directly use pre-generated test data
            result = self.run_single_test(config, test_data, tile_idx)
            results.append(result)
            
            if result['success']:
                print(f"✅ {result['us_time']:.2f} us, {result['tflops']:.2f} tflops, "
        ...
        print(f"{'Rank':<6} {'Tile Config':<20} {'Time(us)':<12} {'Throughput(tflops)':<12} {'Bandwidth(GB/s)':<12}")
        print(f"{'-'*68}")
        ...
        for rank, result in enumerate(successful_results[:3], 1):
            ...
            print(f"{rank:<6} {tile_str:<20} {result['us_time']:<12.2f} {result['tflops']:<12.2f} {result['bw']:<12.2f}")
    ...
    def tune_all(self):
        ...
        print(f"{'='*80}")
        print(f"Successful configurations: {len(all_best_configs)}/{len(self.untuned_configs)}")
        print(f"Results saved to: {self.output_csv}")
        print(f"{'='*80}\n")
        
        # Display summary
        print("📊 All Configurations Summary:")
        print(f"{'Token':<8} {'Model':<8} {'Inter':<8} {'Expert':<8} {'TopK':<6} {'Best Tile':<20} {'Time(us)':<12} {'Throughput(tflops)':<12} {'Bandwidth(GB/s)':<12}")
        print(f"{'-'*112}")
        ...
        for config in all_best_configs:
            ...
            print(f"{config['token']:<8} {config['model_dim']:<8} {config['inter_dim']:<8} "
...
def check_files(args):
    ...
    if not os.path.exists(args.candidate):
        print(f"❌ Error: Candidate configuration file does not exist: {args.candidate}")
        sys.exit(1)
...
def print_results(args, success):
    ...
    print("  python op_tests/test_moe_2stage.py -t <token> -dim <dim> -e <expert> -k <topk> -q 2 -a silu")
    print("=" * 80)
