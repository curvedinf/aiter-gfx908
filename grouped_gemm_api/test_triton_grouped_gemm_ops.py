"""
Unit tests for triton_grouped_gemm_ops: grouped_gemm_fprop / grouped_gemm_dgrad /
grouped_gemm_wgrad.

Each test compares against a pure-PyTorch reference that loops over groups.
Structure mirrors test_grouped_gemm_api.py so the two backends are tested
identically.
"""
...
class TestTritonGroupedGemmAPI:
...
class TestWgradOutputAccum:
    ...
    The triton implementation uses nptgmm's native accumulate=True path,
    which adds the freshly-computed gradient directly into existing_out inside
    the kernel. result is wgrad (same tensor object) must hold.
    """
    ...
    def test_output_accum(self, G, M, N, K, dtype):
        ...
        # Fresh wgrad (baseline)
        dw1 = grouped_gemm_wgrad(dy, x, split_sizes)

        # Accumulate onto a pre-filled tensor
        ...
        assert result is existing, "output_accum should return the same tensor object"
        expected = existing_clone + dw1
        snr = _compute_snr(expected, result)
        assert snr > 40, f"output_accum SNR {snr:.1f} dB too low"

    def test_output_accum_false_ignores_wgrad(self, G, M, N, K, dtype):
        """When output_accum=False, wgrad argument is ignored and a new tensor is returned."""
        device = "cuda"
        split_sizes = _make_split_sizes(G, M, True, device)
        GM = int(split_sizes.sum().item())
        ...
        result = grouped_gemm_wgrad(dy, x, split_sizes, wgrad=existing, output_accum=False)

        # existing must be untouched
        torch.testing.assert_close(existing, existing_clone)
        # result should match the reference (no accumulation)
        ref = _ref_wgrad(dy, x, split_sizes)
        snr = _compute_snr(ref, result)
        assert snr > 40, f"SNR {snr:.1f} dB too low when output_accum=False"


class TestEdgeCases:
    """Edge cases: single group, int32 split_sizes."""
    ...
    def test_int32_split_sizes(self):
        """split_sizes passed as int32 should be accepted (auto-cast to int32 internally)."""
        device = "cuda"
        dtype = torch.bfloat16
    ...
    def test_int64_split_sizes(self):
        """split_sizes passed as int64 should be accepted (auto-cast to int32 internally)."""
        device = "cuda"
        dtype = torch.bfloat16
        G, M, N, K = 4, 128, 1024, 512
...
class TestAutogradIntegration:
    ...
    Uses the triton implementations for all three passes and compares
    each gradient against the pure-PyTorch reference.
    """
