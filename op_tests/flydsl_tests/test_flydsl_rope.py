...
DEVICE = "cuda:0"
BLOCK_SIZE = 16
X_SIZE = 8  # non-flash cache interleave factor (matches Triton test)
ATOL = 1e-1
RTOL = 1e-1
...
def run_test(
    ...
    if flash_layout:
        ...
        value_cache = torch.zeros(
            num_blocks, KH, D, block_size, dtype=dtype, device=DEVICE,
        )
...
def _category_a_params():
    """Configs where the FlyDSL kernel is correct.

    Excludes T>1 combos with known soffset bugs:
    QH=64, QH==KH (Qr=1), KH=1 with large Qr.
    """
    combos = []
    for T in [1, 2, 4, 128]:
        for QH_per_KH in [2, 4, 16]:
            for KH in [1, 8]:
                for D in [64, 128]:
                    QH = QH_per_KH * KH
                    if T > 1 and (QH == 64 or QH == KH or (KH == 1 and QH > 8) or D == 64):
                        continue
                    combos.append((T, QH_per_KH, KH, D))
    return combos


@pytest.mark.skipif(not HAS_FLYDSL, reason="FlyDSL not installed")
@pytest.mark.parametrize(
    "T,QH_per_KH,KH,D", _category_a_params(),
    ids=[f"T{t}-Qr{q}-KH{k}-D{d}" for t, q, k, d in _category_a_params()],
)
def test_flydsl_kernel_direct(T, QH_per_KH, KH, D):
...
def test_wrapper_triton_aligned(T, QH_per_KH, KH, D, reuse_freqs_front_part):
    ...
    run_test(
        T=T, QH_per_KH=QH_per_KH, KH=KH, D=D,
        rotate_style=RotateStyle.NEOX,
        reuse_freqs_front_part=reuse_freqs_front_part,
        flash_layout=True,
        use_wrapper=True,
...
@pytest.mark.skipif(not HAS_FLYDSL, reason="FlyDSL not installed")
def test_wrapper_fallback_f16():
    """Category B: f16 with T>1 → Triton fallback."""
    run_test(
        T=4, QH_per_KH=16, KH=1, D=128,
        rotate_style=RotateStyle.NEOX,
        flash_layout=True,
        dtype=torch.float16,
        use_wrapper=True,
...
def test_flydsl_vs_aiter_triton(T, QH_per_KH, KH, D):
    ...
    q, k, _, _, freqs, positions, offsets, cos, sin = generate_rope_inputs(
        B=1, S=T, H=KH, Q=QH_per_KH, D=D,
        cached=True, reuse_freqs_front_part=False,
        nope=False, pos=True, offs=False,
        two_inputs=True, layout="thd", dtype=torch.bfloat16,
    )
    v = torch.randn_like(k)
    ...
    q_fly, k_fly, _, _ = flydsl_fused_qk_rope_reshape_and_cache(
        q.clone(), k.clone(), v, key_cache_fly, value_cache_fly,
        slot_mapping.to(torch.int64), positions.to(torch.int64),
    ...
    q_tri, k_tri, _, _ = fused_qk_rope_reshape_and_cache(
        q.clone(), k.clone(), v, key_cache_tri, value_cache_tri,
        slot_mapping, positions, cos, sin, k_scale, v_scale,
        is_neox=True, flash_layout=True,
        apply_scale=False, output_zeros=False,
    )
    torch.cuda.synchronize()
