...
def test_split_ar_rmsnorm(tp_size, pp_size, shape, dtype, withGraph=False):
    ...
    for i in range(tp_size):
        ...
        rets.append(
            pool.apply_async(split_ar_rmsnorm, args=(tp_size, pp_size, i, x, weight, eps, withGraph))
...
def test_fused_ar_rmsnorm(tp_size, pp_size, shape, dtype, withGraph=False):
    ...
    for i in range(tp_size):
        ...
        weight_list.append(weight)
        rets.append(
            pool.apply_async(fused_ar_rmsnorm, args=(tp_size, pp_size, i, x, weight, eps, withGraph))
            # pool.apply_async(run_cu, args=(x, weight, eps, i))
        )
    pool.close()
    pool.join()
...
def acc_test(tp_size, pp_size, shape, dtype):
    ...
    for i in range(tp_size):
        ...
        weight_list.append(weight)
        rets.append(
            pool.apply_async(get_acc_value_only, args=(tp_size, pp_size, i, x, weight, eps, 1))
        )
    pool.close()
    pool.join()
...
def acc_test_cudagraph_on(tp_size, pp_size, shape, dtype, loop_time = 1):
    ...
    for i in range(tp_size):
        ...
        weight_list.append(weight)
        rets.append(
            pool.apply_async(get_acc_value_with_cudagraph, args=(tp_size, pp_size, i, x, weight, eps, loop_time))
        )
    pool.close()
    pool.join()
