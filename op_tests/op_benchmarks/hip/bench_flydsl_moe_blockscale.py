...
# ---------------------------------------------------------------------------
def _bench_ck(data, fly_out1, a2_bq, a2_scale_2d, *, num_iters, num_warmup,
              ck_block_m=32):
...
def _bench_asm(data, sorted_ids_a, sorted_w_a, sorted_e_a, num_valid_a,
               *, num_iters, num_warmup):
    tokens = data["tokens"]; model_dim = data["model_dim"]; topk = data["topk"]
    blk_n, blk_k = SCALE_BLOCK_N_DEFAULT, SCALE_BLOCK_K_DEFAULT
    ...
    def _run_asm():
        ...
        aiter.fmoe_fp8_blockscale_g1u1(
            out_asm, data["a1_bq"], data["w1_bq_shuf"], data["w2_bq_shuf"],
...
# ---------------------------------------------------------------------------
def _bench_one(preset_name: str, shape: dict, M: int, *,
               tile: tuple[int, int, int], waves_per_eu: int, act: str,
               compare: set[str], num_iters: int, num_warmup: int):
    ...
    try:
        ...
        return [{
            "preset": preset_name, "M": M, "kernel": "data-prep",
            "stage": "-", "us": float("nan"), "tflops": 0.0,
    ...
    try:
        out1, a1_bq, _ref, sorted_ids, sorted_w, sorted_e, num_valid, us_fly_s1 = \
            _launch_flydsl_stage1(
                ...
                waves_per_eu=waves_per_eu, act=act,
            )
        out2, a2_bq, a2_scale_2d, us_fly_s2 = _launch_flydsl_stage2(
            data, out1, tile_m=tile_m, tile_n=tile_n, tile_k=tile_k,
            waves_per_eu=waves_per_eu, sorted_ids=sorted_ids,
            sorted_w=sorted_w, sorted_e=sorted_e, num_valid=num_valid,
        )
        us_fly_total = us_fly_s1 + us_fly_s2
        fly_note = ""
    except (NotImplementedError, RuntimeError, AssertionError) as e:
        return [{
            "preset": preset_name, "M": M, "kernel": "FlyDSL",
            "stage": "-", "us": float("nan"), "tflops": 0.0,
            "vs_flydsl_total": float("nan"),
    ...
    rows = [
        {"preset": preset_name, "M": M, "kernel": "FlyDSL", "stage": "s1",
         "us": us_fly_s1, "tflops": _tflops(flops_s1, us_fly_s1),
         "vs_flydsl_total": float("nan"), "note": fly_note},
        {"preset": preset_name, "M": M, "kernel": "FlyDSL", "stage": "s2",
         "us": us_fly_s2, "tflops": _tflops(flops_s2, us_fly_s2),
         "vs_flydsl_total": float("nan"), "note": ""},
        {"preset": preset_name, "M": M, "kernel": "FlyDSL", "stage": "total",
         "us": us_fly_total, "tflops": _tflops(flops_total, us_fly_total),
         "vs_flydsl_total": 1.0, "note": ""},
    ]
    ...
    if "ck" in compare:
        try:
            us_ck_s1, us_ck_s2, sorted_ids_a, sorted_w_a, sorted_e_a, num_valid_a = \
                _bench_ck(data, out1, a2_bq, a2_scale_2d,
                          num_iters=num_iters, num_warmup=num_warmup)
            us_ck_total = us_ck_s1 + us_ck_s2
            rows.append({"preset": preset_name, "M": M, "kernel": "CK", "stage": "s1",
                         "us": us_ck_s1, "tflops": _tflops(flops_s1, us_ck_s1),
                         "vs_flydsl_total": us_ck_s1 / us_fly_s1, "note": ""})
            rows.append({"preset": preset_name, "M": M, "kernel": "CK", "stage": "s2",
                         "us": us_ck_s2, "tflops": _tflops(flops_s2, us_ck_s2),
                         "vs_flydsl_total": us_ck_s2 / us_fly_s2, "note": ""})
            rows.append({"preset": preset_name, "M": M, "kernel": "CK", "stage": "total",
                         "us": us_ck_total, "tflops": _tflops(flops_total, us_ck_total),
                         "vs_flydsl_total": us_ck_total / us_fly_total, "note": ""})
        except Exception as e:
            rows.append({"preset": preset_name, "M": M, "kernel": "CK", "stage": "total",
                         "us": float("nan"), "tflops": 0.0,
                         "vs_flydsl_total": float("nan"),
                         "note": f"skipped ({type(e).__name__}: {str(e)[:120]})"})
    ...
    if "asm" in compare:
        try:
            ...
            us_asm = _bench_asm(data, sorted_ids_a, sorted_w_a, sorted_e_a, num_valid_a,
                                num_iters=num_iters, num_warmup=num_warmup)
            rows.append({"preset": preset_name, "M": M, "kernel": "ASM", "stage": "fused",
                         "us": us_asm, "tflops": _tflops(flops_total, us_asm),
                         "vs_flydsl_total": us_asm / us_fly_total, "note": ""})
        except Exception as e:
            rows.append({"preset": preset_name, "M": M, "kernel": "ASM", "stage": "fused",
                         "us": float("nan"), "tflops": 0.0,
                         "vs_flydsl_total": float("nan"),
...
def _print_block(preset_name: str, shape: dict, M: int, rows: list[dict]):
    ...
    print(f"  {'kernel':<8s} {'stage':<6s} | {'us':>10s} | {'TFLOPS':>9s} | {'vs FlyDSL':>10s}  note")
    ...
    for r in rows:
        ...
        print(f"  {r['kernel']:<8s} {r['stage']:<6s} | {us_s} | {tf_s} | {vs_s}  {r['note']}")


def _write_csv(out_path: Path, all_rows: list[dict], tile, waves_per_eu, act):
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = [
        "preset", "M", "model_dim", "inter_dim", "experts", "topk",
        "tile_m", "tile_n", "tile_k", "waves_per_eu", "act",
        "kernel", "stage", "us", "tflops", "vs_flydsl_total", "note",
    ]
    with out_path.open("w", newline="") as f:
        ...
        for r in all_rows:
            ...
            w.writerow({
                "preset": r["preset"], "M": r["M"],
                "model_dim": shape["model_dim"], "inter_dim": shape["inter_dim"],
                "experts": shape["experts"], "topk": shape["topk"],
                "tile_m": tile[0], "tile_n": tile[1], "tile_k": tile[2],
                "waves_per_eu": waves_per_eu, "act": act,
                "kernel": r["kernel"], "stage": r["stage"],
                "us": r["us"], "tflops": r["tflops"],
                "vs_flydsl_total": r["vs_flydsl_total"], "note": r["note"],
            })
    print(f"\nWrote CSV: {out_path}")
...
def parse_args(argv=None):
    ...
    p.add_argument("--preset", default="dsr1",
                   help=f"Shape preset name, or 'all'. Options: "
                        f"{', '.join(PRESETS)}, all")
    p.add_argument("--shapes-csv", type=Path, default=None,
    ...
    p.add_argument("--tile", type=int, nargs=3, default=[64, 128, 128],
    ...
    p.add_argument("--compare", default="ck,asm",
...
def main(argv=None):
    ...
    if args.shapes_csv:
        ...
        if args.preset not in PRESETS:
            print(f"ERROR: unknown preset {args.preset!r}; "
                  f"choose from {list(PRESETS)} or 'all'", file=sys.stderr)
            return 2
    ...
    for preset_name in preset_names:
        ...
        for M in M_list:
            rows = _bench_one(
                preset_name, shape, M,
                tile=tuple(args.tile), waves_per_eu=args.waves_per_eu,
                act=args.act, compare=compare,
