...
python bench_mqa.py -b 4 -hq 32 -hk 1 -sq 2048 -sk 2048 -d 128 -print_vgpr > $OUTPUT_DIR/vgpr_usage.txt 2>&1

echo "Benchmarking complete! Results saved in $OUTPUT_DIR/"
echo "Total combinations tested: $CURRENT"
```

Save this script as `bench_mqa_comprehensive.sh`, make it executable, and run it:

```bash
chmod +x bench_mqa_comprehensive.sh
./bench_mqa_comprehensive.sh
```
