...
def calculate_tflops(start_inds, end_inds, num_heads_q, head_dim, time_ms):
    ...
    total_flops = (
        2.0 * num_heads_q * head_dim * 1024 * 1024 // 2
    )
...
def run_benchmark(args):
    ...
    def bench_fp8_mqa_logits(seq_q_l, seq_kv_l, num_heads_q, head_dim, metric, **kwargs):
        q = torch.randn(seq_q_l, num_heads_q, head_dim, device='cuda', dtype=torch.bfloat16)
        ...
        ke = torch.arange(seq_q_l, dtype=torch.int, device='cuda') + (seq_kv_l - seq_q_l)
...
def main():
    ...
    parser.add_argument('--seq_q_l', type=int, default=4096, help='Input sequence length')
    parser.add_argument('--seq_kv_l', type=int, default=4096, help='Output sequence length')
    parser.add_argument(
