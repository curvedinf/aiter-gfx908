...
def bench_gemm_fn(M: int, N: int, K: int, metric: str, layout: str, impl: callable, use_torch: bool = False):
    ...
    if use_torch:
        ms = triton.testing.do_bench(
            lambda: run_torch(x, weight, x_scale, w_scale, c_dtype),
            warmup=25,
            rep=100,  # noqa: E731
        )
    else:
...
def run_model_benchmark(args, impl):
    ...
    def bench_gemm_a8w8_blockscale(
        ...
        return bench_gemm_fn(M, N, K, metric, args.layout, impl, use_torch=(provider[0]=="torch"))
...
def run_shape_benchmark(args, impl):
    ...
    @triton.testing.perf_report([benchmark])
    def bench_gemm_a8w8_blockscale(M, N, K, metric, provider, model_name=None, **kwargs):
        # Divide N by tensor parallel
        N = math.ceil(N / args.tp)
        return bench_gemm_fn(M, N, K, metric, args.layout, impl, use_torch=(provider=="torch"))
