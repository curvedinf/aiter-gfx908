...
def calculate_mem_bw(batch_size, seq_q_l, seq_kv_l, num_heads_q, num_heads_k, head_size, block_size, window_size, time_us, q_fp8, kv_fp8):
    if window_size > 0:
...
def create_configs():
    x_names = ['num_heads_q', 'num_heads_k', 'head_size', 'seq_q_l', 'seq_kv_l', 'bs','window_size', 'block_size', 'shuffled_kv_cache', 'q_fp8', 'kv_fp8', 'use_tdm', 'num_kv_blocks']
    x_vals = []
    for q_heads in [64, 8]:
        for seq_l in [1024, 2048, 4096, 8192]:
            x_vals.append([q_heads, q_heads // 8, 64, seq_l, seq_l, 1, 0, 64, 0, 0, 0, 1, 1])
...
def run_benchmark(configs):
    ...
    def benchmark(num_heads_q, num_heads_k, head_size, seq_q_l, seq_kv_l, bs, 
                window_size, block_size, shuffled_kv_cache, q_fp8, kv_fp8,
                use_tdm, num_kv_blocks, provider):
        ...
        (maybe_quantized_query, maybe_quantized_key_cache, maybe_quantized_value_cache, 
                    ...
                    window_size_tuple,
                    block_tables,
                    q_descale, k_descale, v_descale, output_scale) = generate_data(seq_lens, num_blocks=1024, block_size=block_size, head_size=head_size, 
                                                                    num_heads=(num_heads_q, num_heads_k), sliding_window=window_size,
                                                                    q_dtype=q_dtype, kv_dtype=kv_dtype, shuffled_kv_cache=shuffled_kv_cache)
        output_scale = None
        soft_cap = None
        ...
        perf = lambda ms: calculate_tflops(bs, seq_q_l, seq_kv_l, num_heads_q, num_heads_k, head_size, window_size, ms * 1000.0)
