...
Provider registry → preset grids → model configs
→ create_benchmark_configs → run_benchmark → CLI. Differences: HK < HQ with HQ % HK == 0,
and --model rows skip configs that are full MHA (HK == HQ) or invalid.
"""
...
# --- GQA head constraints (bench_mha.py is the HQ == HK case; this script enforces HK < HQ) ---


def _validate_gqa(hq: int, hk: int) -> None:
    """Hard requirements for tensors passed to flash_attn_* in GQA mode."""
    assert hq > 2, f"GQA requires HQ > 2; got HQ={hq}"
    assert 1 < hk < hq, f"GQA requires 1 < HK < HQ; got HK={hk}, HQ={hq}"
    assert hq % hk == 0, f"GQA requires HQ divisible by HK; got HQ={hq}, HK={hk}"


def _default_gqa_kv_heads(hq: int) -> int:
    """Default HK when -hk is omitted (custom mode only; --model uses JSON).

    Prefer a small group size (HQ//4, HQ//8, HQ//2) so we stay in a typical GQA regime;
    fall back to the largest proper divisor of HQ strictly between 1 and HQ.
    """
    if hq <= 2:
        raise ValueError(f"GQA requires HQ > 2; got HQ={hq}")
    for cand in (hq // 4, hq // 8, hq // 2):
        if cand > 1 and cand < hq and hq % cand == 0:
            return cand
    for hk in range(min(hq - 1, hq // 2), 1, -1):
        if hq % hk == 0:
            return hk
    raise ValueError(f"No valid GQA HK for HQ={hq} (need 1 < HK < HQ and HQ % HK == 0)")
...
# Registry populated by create_benchmark_configs, looked up by bench_gqa
...
def nonvarlen_benchmark_configs():
    """bshd grid: (BATCH, HQ, HK, N_CTX_Q, N_CTX_K)."""
    batch_sizes = [1, 4, 16]
    hq_hk_pairs = [(32, 8), (32, 4), (48, 6), (48, 8), (64, 8)]
    seq_len_q = [1, 1024, 4096]
    seq_len_k = [163, 8192]
    configs = []
    for batch_size, (hq, hk), sq, sk in itertools.product(
        batch_sizes, hq_hk_pairs, seq_len_q, seq_len_k
    ):
        _validate_gqa(hq, hk)
        configs.append((batch_size, hq, hk, sq, sk))
    return configs


def varlen_benchmark_configs():
    """thd grid: same x-axis semantics as nonvarlen_benchmark_configs (token layout differs in run)."""
    batch_sizes = [1, 4, 8]
    hq_hk_pairs = [(32, 8), (32, 4), (48, 6), (64, 8)]
    seq_len_q = [1, 1024, 4096]
    seq_len_k = [163, 8192]
    configs = []
    for batch_size, (hq, hk), sq, sk in itertools.product(
...
def model_benchmark_configs(args):
    """Build sweep rows from model_configs.json (e.g. num_attention_heads, num_key_value_heads).

    bench_mha.py accepts every model; here we only keep true GQA: 1 < HK < HQ and HQ % HK == 0.
    MHA entries (HK == HQ) or bad splits are skipped with a warning.
    """
    ...
    if skipped:
        warnings.warn(
            "Skipping non-GQA models (need 1 < HK < HQ and HQ % HK == 0): "
            + ", ".join(skipped)
        )
    return fa_configs
...
def create_benchmark_configs(custom, args):
    """Mirror bench_mha.create_benchmark_configs: dtype → x grid → providers → triton Benchmark.

    GQA-only deltas: custom row uses _default_gqa_kv_heads when -hk is 0; --model adds D_HEAD*
    columns and filters non-GQA configs in model_benchmark_configs.
    """
    dtype = arg_to_torch_dtype[args.tensor_dtype]
    # bench_mha.py: hk = args.hq if not args.hk else args.hk
    # GQA custom sweep: omitting -hk picks a valid HK < HQ; other modes get (HQ, HK) from the grid/JSON.
    hk = _default_gqa_kv_heads(args.hq) if custom and not args.hk else args.hk
    ...
    if custom:
        ...
        if args.model:
            ...
            plot_name = f"gqa-attention-{mode}-layout-{args.layout}-dtype-{args.dtype}-causal-{causal}"
            extra_args = {"dtype": dtype, "causal": causal, "mode": mode}
            if not x_vals_list:
                raise ValueError(
                    "No GQA model configs: selected model(s) are not GQA "
                    "(need 1 < HK < HQ and HQ % HK == 0)."
                )
...
def run_benchmark(custom, args):
    ...
    def bench_gqa(
        ...
        # Generate base inputs: Q has HQ heads; K/V use HK (GQA). FLOP estimate uses HQ (query heads).
        q = torch.randn(
        ...
        # Input preparation (bshd vs thd): same structure as bench_mha.run_benchmark
        if varlen:
    ...
    try:
        bench_gqa.run(save_path=args.o, print_data=True)
...
def parse_args(args: list[str] | None = None) -> argparse.Namespace:
    parser = get_parser(kernel_name="GroupedQueryAttention (GQA)")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="KV heads (GQA: 1 < HK < HQ, HQ %% HK == 0). Default: derived from HQ (e.g. HQ//4).",
    )
...
def post_process_args(args: argparse.Namespace) -> tuple[argparse.Namespace, bool]:
    # Match bench_mha.post_process_args: --model pins causal+thd unless user overrides.
    if args.model:
    ...
    if args.hq or args.hk or args.d or args.dv:
        ...
        assert (
            args.b and args.hq and args.sq and args.d and args.dv
                ...
                and head size."
        if args.hk == 0:
            args.hk = _default_gqa_kv_heads(args.hq)
        _validate_gqa(args.hq, args.hk)
