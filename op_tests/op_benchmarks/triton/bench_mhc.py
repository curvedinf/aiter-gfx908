...
def run_benchmark(args):
    ...
    def bench_mhc_kernel(M, n, C, provider):
        ...
        if hres_mode == "lite":
            # mHC-lite: softmax(logits) @ P
            # Softmax FLOPs per element (log-domain implementation):
            #   1. max reduction: ~1 op
            #   2. subtract max: 1 op
            #   3. multiply + exp2: 2 ops
            #   4. sum reduction: ~1 op
            #   5. log2 + multiply + add: 3 ops
            #   6. subtract: 1 op
            #   7. multiply + exp2: 2 ops
            #   Total: ~10-11 ops per element
