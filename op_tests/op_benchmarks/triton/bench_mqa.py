...
def nonvarlen_benchmark_configs():
    ...
    seq_len_k = [163, 8192]
    # For MQA, HK is always 1
    configs = list(itertools.product(batch_sizes, N_HEADS, seq_len_q, seq_len_k))
    configs = [
        (batch_size, N_HEAD, 1, seq_len_q, seq_len_k)  # HK=1 for MQA
...
def create_benchmark_configs(custom, args):
    ...
    # For MQA, HK is always 1, but allow override for testing
    hk = 1 if not args.hk else args.hk
    if args.hk and args.hk != 1:
        warnings.warn(
            f"MQA benchmark expects HK=1, but got HK={args.hk}. This may not be true MQA.",
    ...
    if custom:
        ...
        if args.model:
            ...
            plot_name = f"mqa-attention-{mode}-layout-{args.layout}-fp8-{args.fp8}-causal-{causal}"
...
def run_benchmark(custom, args):
    ...
    def bench_mqa(
        ...
        """
        Benchmark or test function for multi-query attention (MQA).
        ...
        # MQA validation: HK should be 1
        if HK != 1:
            warnings.warn(
                f"MQA benchmark expects HK=1, but got HK={HK}. This may not be true MQA.",
                category=RuntimeWarning,
            )
        assert (
            HQ % HK == 0
        ), f"For MQA/GQA, HQ ({HQ}) must be divisible by HK ({HK})"
        ...
        if args.test_mode:
            ...
            print(
                f"Testing MQA kernel implementation <{provider}> against Torch with shape:"
            )
            print(
        ...
        if varlen:
            ...
            for i in range(num_contexts):
                ...
                if causal:
                    ...
                    # For MQA, we compute with HQ heads but only HK=1 KV heads
...
def parse_args(args: list[str] | None = None) -> argparse.Namespace:
    parser = get_parser(kernel_name="Multi-Query Attention (MQA)")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of key/value heads. For MQA, should be 1 (default).",
    )
...
def post_process_args(args: argparse.Namespace) -> tuple[argparse.Namespace, bool]:
    ...
    if args.hq or args.hk or args.d or args.dv:
        ...
        if not args.dv:
            args.dv = args.d
        # For MQA, default HK to 1 if not specified
        if not args.hk:
            args.hk = 1
        assert (
