...
usage() {
  cat <<'EOF'
Run op_tests/op_benchmarks/triton/bench_moe_gemm_a8w4.py across a sweep and MERGE the benchmark's
...
Sweepable CLI parameters in bench_moe_gemm_a8w4.py:
  ...
  --act-dtype <fp8|mx8>   (optional; default "fp8")
...
Usage:
  ./run_bench_moe_gemm_a8w4.sh -o <output.csv> [--keep-tmp]

Example:
  ./run_bench_moe_gemm_a8w4.sh -o bench_out/bench_moe_gemm_a8w4_merged.csv
...
BENCH="${AITER_REPO}/op_tests/op_benchmarks/triton/bench_moe_gemm_a8w4.py"
...
write_header_once() {
  ...
  printf "timestamp,shape_dim1,shape_dim2,experts_total,experts_active,act_dtype,num_weight_inits,op_regex,m_mode,M_list,%s\n" "${bench_header}" > "${OUT_FILE}"
}
...
append_csv_body() {
  ...
  | awk -v ts="${ts}" -v d1="${dim1}" -v d2="${dim2}" -v te="${total_ex}" -v ae="${active_ex}" \
        -v ad="${act_dtype}" -v nwi="${nwi}" -v rx="${op_regex}" -v mm="${m_mode}" -v ml="${m_list_str}" '
      BEGIN{OFS=","}
      { print ts, d1, d2, te, ae, ad, nwi, rx, mm, ml, $0 }
    ' >> "${OUT_FILE}"
}
...
# ----------------------------
total_runs=$(( ${#SHAPES_LIST[@]} * ${#EXPERTS_LIST[@]} * ${#ACT_DTYPE_LIST[@]} * ${#NUM_WEIGHT_INITS_LIST[@]} * ${#OP_REGEX_LIST[@]} * ${#M_MODE_LIST[@]} ))
run_idx=0
      ...
      for nwi in "${NUM_WEIGHT_INITS_LIST[@]}"; do
        for op_regex in "${OP_REGEX_LIST[@]}"; do
          for m_mode in "${M_MODE_LIST[@]}"; do
            ...
            echo "RUN shape=${dim1}x${dim2} experts=${total_ex}/${active_ex} act_dtype=${act_dtype} num_weight_inits=${nwi} m_mode=${m_mode}"
            ...
            if [[ "${#csvs[@]}" -eq 0 ]]; then
              ...
              for bench_csv in "${csvs[@]}"; do
                ...
                append_csv_body "${ts}" "${dim1}" "${dim2}" "${total_ex}" "${active_ex}" "${act_dtype}" "${nwi}" "${op_regex}" "${m_mode}" "${m_list_str}" "${bench_csv}"
