...
from aiter.ops.triton.attn_qk_int8_per_block import (
    attn_qk_int8_per_block as attn_false,
    per_block_int8,
)
...
def sageattn(
    ...
    **kwargs: Any,
    ...
    # q_int8, q_scale, k_int8, k_scale, _ = per_block_int8(q, k, sm_scale=sm_scale, tensor_layout=tensor_layout, smooth_k=smooth_k)
    ...
    if is_causal:
        ...
        if attn_mask is not None:
            ...
            try:
                ...
                raise AssertionError(f"attn_mask shape {attn_mask.shape} cannot be broadcast to {target_shape}")
        o = attn_false(q, k, v, tensor_layout=tensor_layout, attn_mask=attn_mask, output_dtype=dtype)
