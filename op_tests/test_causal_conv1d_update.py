...
def torch_causal_conv1d_update_ref(
...
def test_multiuser_streaming(dtype=torch.float32):
    ...
    for seqlen in step_seqlens:
        bytes_read = (
            batch * dim * seqlen * dtype.itemsize
            + batch * dim * state_len * dtype.itemsize
            + dim * width * dtype.itemsize
            + dim * dtype.itemsize
        )
        bytes_write = (
            batch * dim * seqlen * dtype.itemsize
...
def test_causal_conv1d_update_pytest_params(dtype=torch.bfloat16):
    ...
    conv_state = torch.randn(
        batch, state_len, dim, device="cuda", dtype=dtype
    ).transpose(-1, -2)
    ...
    modes_tested.append(
        {"mode": "non-circular", "err": err1, "us": us_aiter1, "passed": err1 <= 0.1}
    )
    ...
    modes_tested.append(
        {"mode": "circular", "err": err2, "us": us_aiter2, "passed": err2 <= 0.1}
    )
...
@benchmark()
def test_causal_conv1d_update_continuous_batching(
    batch, dim, seqlen, width, state_len, dtype, use_silu=False
):
    """
...
parser.add_argument(
    "--seqlen", type=int, default=None, help="Override seqlen (for single-step tests)"
)
