...
struct ConvParamsBaseUpdate {
    ...
    index_t q_out_batch_stride;
    index_t q_out_c_stride;
    index_t q_out_l_stride;
    
    index_t k_out_batch_stride;
    index_t k_out_c_stride;
    index_t k_out_l_stride;
    
    index_t v_out_batch_stride;
    index_t v_out_c_stride;
    index_t v_out_l_stride;
    ...
    void *__restrict__ k_out_ptr;
    void *__restrict__ v_out_ptr;
...
void causal_conv1d_update_split_qkv_kernel(ConvParamsBaseUpdate params) {
    ...
    // Determine which output (Q, K, or V) this channel belongs to
    input_t *out;
    int local_channel_id;
    if (channel_id < params.q_dim) {
        // Q output
        local_channel_id = channel_id;
        out = reinterpret_cast<input_t *>(params.q_out_ptr) + batch_id * params.q_out_batch_stride
            + local_channel_id * params.q_out_c_stride;
    } else if (channel_id < params.q_dim + params.k_dim) {
        // K output
        local_channel_id = channel_id - params.q_dim;
        out = reinterpret_cast<input_t *>(params.k_out_ptr) + batch_id * params.k_out_batch_stride
            + local_channel_id * params.k_out_c_stride;
    } else {
        // V output
        local_channel_id = channel_id - params.q_dim - params.k_dim;
        out = reinterpret_cast<input_t *>(params.v_out_ptr) + batch_id * params.v_out_batch_stride
            + local_channel_id * params.v_out_c_stride;
    }
    ...
    for (int i = 0; i < params.seqlen; ++i) {
        ...
        // Determine stride based on which output this is
        int out_l_stride;
        if (channel_id < params.q_dim) {
            out_l_stride = params.q_out_l_stride;
        } else if (channel_id < params.q_dim + params.k_dim) {
            out_l_stride = params.k_out_l_stride;
        } else {
            out_l_stride = params.v_out_l_stride;
        }
        
        out[i * out_l_stride] = input_t(out_val);
...
void cpu_causal_conv1d_update_split_qkv_fp32(
    ...
    float* q_out,             // [batch, q_dim, seqlen]
    float* k_out,             // [batch, k_dim, seqlen]
    float* v_out,             // [batch, v_dim, seqlen]
...
int run_test(const char* dtype_name) {
    ...
    const int k_dim = 512;
    const int v_dim = 512;
    const int dim = q_dim + k_dim + v_dim;  // Total dimension
    const int seqlen = 1;
    const int width = 4;
    ...
    std::cout << "=== Causal Conv1D Update Kernel Test (Split QKV) ===" << std::endl;
    std::cout << "Configuration:" << std::endl;
    std::cout << "  Batch: " << batch << std::endl;
    std::cout << "  Q Dim: " << q_dim << std::endl;
    std::cout << "  K Dim: " << k_dim << std::endl;
    std::cout << "  V Dim: " << v_dim << std::endl;
    ...
    params.q_out_batch_stride = q_dim * seqlen;
    params.q_out_c_stride = seqlen;
    params.q_out_l_stride = 1;
    
    params.k_out_batch_stride = k_dim * seqlen;
    params.k_out_c_stride = seqlen;
    params.k_out_l_stride = 1;
    
    params.v_out_batch_stride = v_dim * seqlen;
    params.v_out_c_stride = seqlen;
    params.v_out_l_stride = 1;
    ...
    params.q_out_ptr = d_q_out;
    params.k_out_ptr = d_k_out;
    params.v_out_ptr = d_v_out;
    ...
    std::cout << "Output comparison (Q, K, V combined):" << std::endl;
    ...
    HIP_CHECK(hipFree(d_q_out));
    HIP_CHECK(hipFree(d_k_out));
    HIP_CHECK(hipFree(d_v_out));
