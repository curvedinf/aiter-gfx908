...
def run_ck(
    ...
    print(f"q_unpad shape is {q_unpad.shape}, stride is {q_unpad.stride()}")
    print(f"k_unpad shape is {k_unpad.shape}, stride is {k_unpad.stride()}")
    print(f"v_unpad shape is {v_unpad.shape}, stride is {v_unpad.stride()}")
    if bias is not None:
    ...
    if type(outputs) is tuple:
        ...
        # out = outputs[0].view(1, q_unpad.shape[0], q_unpad.shape[1], d_v)
    else:
        out = output_pad_fn(outputs)
        # out = outputs.view(1, q_unpad.shape[0], q_unpad.shape[1], d_v)
    ...
    if dout is None or not return_lse:
        return out, dropout_mask, None, None, None, (us_fwd, fwd_flop, fwd_num_bytes, None,None,None)
    else:
