...
def test_rmsnorm2d(dtype, m, n):
    ...
    cgbps = ComputeGBps(input.nbytes + weight.nbytes + a.nbytes)
    ...
    msg += f"\n[throughput] dim: {str(dim):<20}, dtype: {dtype}, torch avg: {cgbps(avg_a):<8.2f} GBps, ck avg: {cgbps(avg_b):<8.2f} GBps, cu avg: {cgbps(avg_c):<8.2f} GBps"
