...
"""
Microbenchmark: unified attention 2D vs 3D only, replayed from JSONL shapes.

This script benchmarks all (or top-N) JSONL rows with:
  - unified_force_2d
  - unified_force_3d

and writes a per-row CSV with winner statistics.
"""
...
def make_inputs(case: Case, seed: int, ignore_sinks: bool) -> dict[str, Any]:
    ...
    # Compact physical cache while preserving per-case sequence geometry.
    ...
    compact_blocks = min(compact_blocks, trace_blocks)
    k = torch.randn(
...
def bench_case(inp: dict[str, Any], force_2d: bool, warmup: int, iters: int) -> float:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Benchmark unified 2D vs 3D on JSONL shapes.")
    ap.add_argument("--jsonl", type=Path, required=True)
    ap.add_argument("--max-cases", type=int, default=0, help="0 means all rows.")
    ap.add_argument("--warmup", type=int, default=5)
    ap.add_argument("--iters", type=int, default=20)
    ap.add_argument("--ignore-sinks", action="store_true", help="Drop sinks tensor for apples-to-apples timing.")
    ap.add_argument("--tie-eps-ms", type=float, default=1e-3, help="Tie threshold in milliseconds.")
    ap.add_argument(
        "--out-csv",
        type=Path,
        default=Path("/workspaces/workspace/unified_2d_vs_3d_microbench.csv"),
    )
    args = ap.parse_args()
    ...
    for i, c in enumerate(cases):
        ...
        is_decode = c.max_q == 1
        inp = make_inputs(c, seed=123 + i, ignore_sinks=args.ignore_sinks)
    ...
    print(f"rows={len(cases)} ok={total_ok} tie_eps_ms={args.tie_eps_ms}")
    if total_ok > 0:
        print(
            f"winner overall: 2d={winners['2d']} 3d={winners['3d']} tie={winners['tie']}"
        )
