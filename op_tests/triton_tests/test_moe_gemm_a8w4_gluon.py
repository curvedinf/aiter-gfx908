...
# matmul utilities
from aiter.ops.triton.moe_op_gemm_a8w4 import swizzle_scales, downcast_to_static_fp8
from aiter.ops.triton.moe_op_gemm_a8w4_gluon import moe_gemm_a8w4_gluon, moe_gemm_torch
# numerics utilities
...
@pytest.mark.parametrize(
    ...
    [
        tuple(getattr(case, f.name) for f in fields(Case)) for case in [
            ...
            Case(16, 1024, 1024, "float8_e4m3fn", "mxfloat4_e2m1", 128, 4, hbm_swizzling=True),
            Case(16, 256, 256, "float8_e4m3fn", "mxfloat4_e2m1", 128, 4, hbm_swizzling=True),
            Case(4, 3072, 3072, "float8_e4m3fn", "mxfloat4_e2m1", 128, 4, hbm_swizzling=True),
        ]
    ],
)
...
def test_op(m, n, k, do_gather, do_scatter, has_y_gammas, apply_swiglu, fused_quant, 
    ...
    tri_y = moe_gemm_a8w4_gluon(x_tri, w_tri, x_mx_scales_tri, w_scale_tri, x_static_scale, quant_static_scale, bias_tri, rdata, gindx, sindx, gammas, swizzle_mx_scale, out_dtype, apply_swiglu)
    if not act_mxfp8 and fused_quant:
