...
def main():
    ...
    for idx_str in args.idx.split(","):
        ...
        csv_ck = float(row.get("ck_ua_ms", row.get("csv_ck_ua_ms", 0)))
        csv_t2 = float(row.get("triton_2d_ms", row.get("csv_triton_2d_ms", 0)))
        csv_t3 = float(row.get("triton_3d_ms", row.get("csv_triton_3d_ms", 0)))
        csv_sp = min(csv_t2, csv_t3) / csv_ck if csv_ck > 0 else float("nan")
